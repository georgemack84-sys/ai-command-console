...
export async function GET(request: Request) {
  try {
    ...
    return apiSuccess(buildMissionConsoleView(buildMissionConsoleSeedContext({ executionId })).dependencies);
  } catch (error) {
    return apiError(error, "Unable to load dependency domain.");
  }
}
