...
export async function GET(request: Request, { params }: { params: Promise<{ certification_run_id: string }> }) {
  try {
    ...
    return apiSuccess(getVisibilityCertificationForRequest(request, certification_run_id).warnings);
  } catch (error) {
    return apiError(error, "Unable to load visibility certification warnings.");
  }
}
