...
export async function GET(request: Request, { params }: { params: Promise<{ certification_run_id: string }> }) {
  try {
    ...
    return apiSuccess(getVisibilityCertificationForRequest(request, certification_run_id).evidence_refs);
  } catch (error) {
    return apiError(error, "Unable to load visibility certification evidence.");
  }
}
