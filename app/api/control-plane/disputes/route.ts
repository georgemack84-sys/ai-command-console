...
export async function GET(request: Request) {
  try {
    ...
    return apiSuccess({
      disputeReview: model.disputeReview,
      replayReview: model.replayReview,
      coordinationReview: model.coordinationReview,
    });
  } catch (error) {
    return apiError(normalizeTenantApiError(error), "Unable to load disputes.");
  }
}
