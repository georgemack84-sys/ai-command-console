...
async function loadState(request: Request) {
  ...
  return buildConstitutionalSovereigntyEngine({ executive, resilience, readiness, nowMs });
}

export async function GET(request: Request) {
  try {
    const sovereignty = await loadState(request);
    return apiSuccess({
      assessment: sovereignty.assessment,
      policies: sovereignty.policies,
      enforcement: sovereignty.enforcement,
      lineage: sovereignty.lineage,
    });
  } catch (error) {
    return apiError(normalizeTenantApiError(error), "Unable to load sovereignty state.");
  }
}
