...
export async function POST(request: Request) {
  try {
    ...
    const result = validateAutonomyConstraints({
      ...
      advisoryOnly: true,
      approvalRequired: executive.governancePressure.approvalBacklog > 0,
      containmentRequired: executive.governancePressure.containmentPressure > 0.55,
      disputed: executive.constraints.blockedReasons.length > 0,
      createdAt: Date.now(),
    });
    return apiSuccess({ ok: true, advisoryOnly: true, previewOnly: true, validation: result });
  } catch (error) {
    return apiError(normalizeTenantApiError(error), "Unable to preview autonomy validation.");
  }
}
