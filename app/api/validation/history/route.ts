...
import { buildValidationAuditRecord } from "@/services/validation/validationAudit";
...
export async function GET(request: Request) {
  try {
    ...
    return apiSuccess({
      history: [
        buildValidationAuditRecord({
          ...
          details: [validation.validationState, validation.severity],
          evidenceRefs: [sovereignty.audit.auditId],
    ...
    return apiError(normalizeTenantApiError(error), "Unable to load validation history.");
  }
}
