...
async function loadValidation(request: Request) {
  ...
  return { sovereignty, validation, nowMs };
}

export async function POST(request: Request) {
  try {
    const result = await loadValidation(request);
    return apiSuccess({
      ok: true,
      advisoryOnly: true,
      previewOnly: true,
      validation: result.validation,
      sovereigntyState: result.sovereignty.assessment.sovereigntyState,
    });
  } catch (error) {
    return apiError(normalizeTenantApiError(error), "Unable to preview constitutional validation.");
  }
}
