...
export async function GET(request: Request, { params }: { params: Promise<{ truth_record_id: string }> }) {
  try {
    ...
    return apiSuccess(getLedgerExplorerDetailForRequest(request, truth_record_id).recommendation_decision);
  } catch (error) {
    return apiError(error, "Unable to load recommendation and decision context.");
  }
}
