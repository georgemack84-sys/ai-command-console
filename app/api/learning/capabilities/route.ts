...
/** Read-only capability inspection. This endpoint cannot grant, renew, reactivate, broaden, or revoke authority. */
export async function GET() {
  try {
    ...
    return apiSuccess({ artifacts: await new PrismaCapabilityArtifactRepository(user.workspaceId).listWorkspaceArtifacts() });
  } catch (error) {
    return apiError(error, "Unable to load capability registry artifacts.");
  }
}
