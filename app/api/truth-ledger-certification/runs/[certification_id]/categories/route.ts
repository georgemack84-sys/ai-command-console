...
export async function GET(request: Request, { params }: { params: Promise<{ certification_id: string }> }) {
  try {
    ...
    return apiSuccess(getTruthLedgerCertificationForRequest(request, certification_id).report.category_results);
  } catch (error) {
    return apiError(error, "Unable to load Truth Ledger certification categories.");
  }
}
