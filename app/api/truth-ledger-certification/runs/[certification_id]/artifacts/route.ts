...
export async function GET(request: Request, { params }: { params: Promise<{ certification_id: string }> }) {
  try {
    ...
    return apiSuccess(getTruthLedgerCertificationForRequest(request, certification_id).artifacts);
  } catch (error) {
    return apiError(error, "Unable to load Truth Ledger certification artifacts.");
  }
}
