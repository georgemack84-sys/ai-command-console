...
export async function GET(request: Request, { params }: { params: Promise<{ certification_id: string }> }) {
  try {
    ...
    return apiSuccess(getTruthLedgerCertificationForRequest(request, certification_id).report);
  } catch (error) {
    return apiError(error, "Unable to load Truth Ledger certification report.");
  }
}
