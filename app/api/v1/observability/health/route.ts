...
export async function GET(request: Request) {
  try {
    ...
    const metricSnapshot = await buildMetricSnapshot({ generatedAt, executionId, tenantContext });
    const healthSnapshot = buildSystemHealthSnapshot({
      generatedAt,
      metrics: metricSnapshot.metrics,
    });

    const contract = getRegisteredContract("api.v1.observability.health.response", "1.0.0");
    if (!contract) {
      throw new AppError(500, "API_CONTRACT_MISSING", "Observability health contract is missing.");
    }
    ...
    if (!validated.ok) {
      recordApiMetric("observability.health.outbound_validation_failed", 1, tenantContext);
    ...
    if (healthSnapshot.status !== "HEALTHY") {
      recordApiMetric("observability.health.degraded_response", 1, tenantContext);
    }
    recordApiMetric("observability.health.success", 1, tenantContext);
    return apiSuccess(validated.data);
  } catch (error) {
    return apiError(normalizeTenantApiError(error), "Unable to load observability health.");
  }
}
