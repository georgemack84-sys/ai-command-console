...
export async function GET(request: Request) {
  try {
    ...
    const alerts = await evaluateCurrentAlerts({ generatedAt, executionId, tenantContext });
    const contract = getRegisteredContract("api.v1.observability.alerts.response", "1.0.0");
    if (!contract) {
      throw new AppError(500, "API_CONTRACT_MISSING", "Observability alerts contract is missing.");
    }
    ...
    if (!validated.ok) {
      recordApiMetric("observability.alerts.outbound_validation_failed", 1, tenantContext);
    ...
    if (snapshot.healthStatus !== "HEALTHY") {
      recordApiMetric("observability.alerts.degraded_response", 1, tenantContext);
    }
    recordApiMetric("observability.alerts.success", 1, tenantContext);
    return apiSuccess(validated.data);
  } catch (error) {
    return apiError(normalizeTenantApiError(error), "Unable to load observability alerts.");
  }
}
