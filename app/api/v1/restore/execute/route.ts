...
import { restoreSimulationFromLatestSnapshot } from "@/services/continuity/restoreSimulation";
...
export async function POST(request: Request) {
  try {
    ...
    const result = await restoreSimulationFromLatestSnapshot({ tenantContext, snapshotId: parsed.data.snapshotId });
    if (!result.ok) {
      recordApiMetric("restore.execute.blocked", 1, tenantContext);
      throw new AppError(409, result.error.code, result.error.message, result.error.details);
    }
    const contract = getRegisteredContract("api.v1.restore.dry_run.response", "1.0.0");
    if (!contract) {
      throw new AppError(500, "API_CONTRACT_MISSING", "Restore execute contract is missing.");
    }
    const payload = {
    ...
    recordApiMetric("restore.execute.governed_dry_run", 1, tenantContext);
    return apiSuccess(validated.data);
  } catch (error) {
    return apiError(normalizeTenantApiError(error), "Unable to execute restore.");
  }
}
