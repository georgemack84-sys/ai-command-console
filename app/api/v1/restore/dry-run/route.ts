...
export async function POST(request: Request) {
  try {
    ...
    if (!result.ok) {
      recordApiMetric("restore.dry_run.failed", 1, tenantContext);
    ...
    if (!contract) {
      throw new AppError(500, "API_CONTRACT_MISSING", "Restore dry-run contract is missing.");
    }
    const payload = {
    ...
    recordApiMetric("restore.dry_run.success", 1, tenantContext);
    return apiSuccess(validated.data);
  } catch (error) {
    return apiError(normalizeTenantApiError(error), "Unable to simulate restore.");
  }
}
