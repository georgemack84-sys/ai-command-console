...
import { getLatestIntegrityReport } from "@/services/continuity/backupIntegrity";
...
export async function GET(request: Request) {
  try {
    ...
    const report = getLatestIntegrityReport(tenantContext);
    const payload = {
      ready: report.ready,
    ...
    return apiError(normalizeTenantApiError(error), "Unable to load integrity report.");
  }
}
