...
Handwritten source must not use `#nullable disable` or a blanket `#pragma warning disable`. A specific pragma must list diagnostic IDs and include an inline reason; `SuppressMessage` must include `Justification`. Project-level `NoWarn` and `WarningsNotAsErrors` are prohibited. A future exception must be scoped to the smallest file or symbol, identify the exact diagnostic, explain why it is inapplicable, and include a removal condition when temporary.

The null-forgiving operator is likewise prohibited in handwritten source unless an exact mechanically guaranteed invariant is registered by the compiler-policy validator and documented immediately beside the assertion. The only current exceptions are the five required EF Core reference navigations in `IdentityEntities.cs`: `UserRole.User`, `UserRole.Role`, `RolePermission.Role`, `RolePermission.Permission`, and `Session.User`. Their required relationships are configured explicitly by `PropriumDbContext`; the initializer exists for EF materialization, not as a general escape from flow analysis. Removing or adding an exception requires changing both the code-level invariant and the exact validator contract.

## Audit result

The GP-42 audit retained centralized `Nullable=enable` across all seven projects and found no project-level weakening override. Six `#nullable disable` directives remain only in source-controlled EF migration, designer, and model-snapshot files recognized by the narrow generated-source policy. Eleven handwritten null-forgiving assertions were replaced with nullable result modeling, explicit guards, pattern narrowing, or checked reflection; the five documented EF navigation invariants remain. The Release solution compiles with zero warnings and errors without live infrastructure.
