...
`npm run validate:configuration` runs the actual contract plus eight controlled root/Compose failures. Repository validation includes those fixtures. Existing secret scanning separately detects credential patterns and unsafe tracked files. Compose parsing proves every root key has a real consumer, the migration service appends only its operational argument to the image entrypoint, and overrides resolve without starting infrastructure.
