...
import * as route from "@/app/api/advisory/export/route";

describe("advisory export API", () => {
  it("returns a read-only advisory snapshot export", async () => {
    const response = await route.GET();
    const body = await response.json();

    expect(body.ok).toBe(true);
    expect(body.data.exportPolicyVersion).toBe("advisory-snapshot-export/v1");
    expect(body.data.authority).toBe("READ_ONLY");
    expect(body.data.snapshotHash).toMatch(/^sha256:/);
    expect(body.data.snapshotId).toMatch(/^sha256:/);
