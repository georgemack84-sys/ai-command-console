...
let cachedDriftAnalysisInput: DriftAnalysisInput | undefined;

function driftedFoundation(overrides: Partial<RecommendationDriftFoundationInput> = {} ) {
...
function driftAnalysisInput(overrides: Partial<DriftAnalysisInput> = {}): DriftAnalysisInput {
  if (!cachedDriftAnalysisInput) {
    const foundationSource = driftFoundationInput();
    cachedDriftAnalysisInput = Object.freeze({
      request: buildDriftAnalysisRequest({ tenantId: "tenant-alpha", recommendationIds: ["recommendation-beta", "recommendation-alpha"], analysisScope: "FULL", graphVersion: "decision-graph/v1" }),
      foundation: driftedFoundation(),
      impactFoundation: foundationSource.currentImpactFoundation,
      impactAnalysis: sealImpactAnalysis(impactAnalysisInput(alignedPortfolioInput())),
      impactReplay: impactCertificationInput().replay,
      impactCertification: sealImpactCertification(impactCertificationInput()),
      dependencyFoundation: foundationSource.currentDependencyFoundation,
      dependencyAnalysis: foundationSource.currentDependencyAnalysis,
      dependencyReplay: foundationSource.currentDependencyReplay,
      dependencyCertification: foundationSource.currentDependencyCertification,
      portfolio: foundationSource.currentPortfolio,
      relationshipAnalysis: foundationSource.currentRelationshipAnalysis,
      portfolioReplay: foundationSource.currentPortfolioReplay,
      portfolioCertification: foundationSource.currentPortfolioCertification,
      recommendations: foundationSource.currentRecommendations,
    } satisfies DriftAnalysisInput);
  }
  return Object.freeze({ ...cachedDriftAnalysisInput, ...overrides } satisfies DriftAnalysisInput);
}
