...
import {
  ...
  createStrategicReadinessEvidencePath,
  sealStrategicReadiness,
  type StrategicReadinessInput,
...
function graphInput(overrides: Partial<DecisionGraphContractInput> = {}): DecisionGraphContractInput {
  const nodes: DecisionGraphNodeInput[] = [
    { nodeId: "boundary-node", graphId: "graph-510a", nodeType: "CONSTRAINT", tenantId: "tenant-alpha", lineageReference: "lineage-boundary-node" },
    { nodeId: "governance-anchor", graphId: "graph-510a", nodeType: "GOVERNANCE", tenantId: "tenant-alpha", lineageReference: "lineage-governance-anchor" },
    { nodeId: "recommendation-anchor", graphId: "graph-510a", nodeType: "RECOMMENDATION", tenantId: "tenant-alpha", lineageReference: "lineage-recommendation-anchor" },
    { nodeId: "escalation-anchor", graphId: "graph-510a", nodeType: "ESCALATION", tenantId: "tenant-alpha", lineageReference: "lineage-escalation-anchor" },
  ];
...
const proposalNodes = (): readonly ProposalNodeInput[] => [
  { proposalId: "proposal-a", graphId: "graph-510a", tenantId: "tenant-alpha", proposalType: "MISSION", lineageReference: "lineage-proposal-a" },
];
const governanceNodes = (): readonly GovernanceNodeInput[] => [
  { governanceId: "governance-a", graphId: "graph-510a", tenantId: "tenant-alpha", governanceType: "POLICY", lineageReference: "lineage-governance-a" },
];
const escalationNodes = (): readonly EscalationNodeInput[] => [
  { escalationId: "escalation-a", graphId: "graph-510a", tenantId: "tenant-alpha", escalationType: "REVIEW", lineageReference: "lineage-escalation-a" },
];
...
function readinessInput(overrides: Partial<StrategicReadinessInput> = {}): StrategicReadinessInput {
  const base = buildReadinessInputs();
  const request = buildStrategicReadinessRequest({
    ...base,
    recommendationId: "recommendation-anchor",
    tenantId: "tenant-alpha",
    readinessScope: "FULL",
  ...
  return Object.freeze({
    ...
    ...base,
    ...overrides,
  } satisfies StrategicReadinessInput);
}

describe("strategicReadinessGate", () => {
  it("is deterministic and reproduces readiness hashes", () => {
    const input = readinessInput();
    const first = sealStrategicReadiness(input);
    const second = sealStrategicReadiness(input);
    expect(first).toEqual(second);
    expect(first.result.readinessState).toBe("READY");
    expect(first.result.readinessHash).toHaveLength(64);
  });

  it("keeps readiness evidence ordering reproducible across scopes", () => {
    const base = readinessInput();
    const scopes = ["EVIDENCE", "OBSERVABILITY", "GOVERNANCE", "REPLAY", "FULL"] as const;
    for (const readinessScope of scopes) {
      const scoped = readinessInput({ request: { ...base.request, readinessScope } });
      expect(createStrategicReadinessEvidencePath(scoped)).toEqual(
        createStrategicReadinessEvidencePath(scoped),
      );
      expect(sealStrategicReadiness(scoped).result.readinessHash).toBe(
        sealStrategicReadiness(scoped).result.readinessHash,
      );
    }
  });

  it("surfaces missing evidence as OBSERVE", () => {
    const base = readinessInput();
    const observed = sealStrategicReadiness({
      ...base,
      replayEvidence: { ...base.replayEvidence, replayReferences: Object.freeze([]) },
    });
    expect(observed.result.readinessState).toBe("OBSERVE");
    expect(observed.validation.reasonCodes).toContain("EVIDENCE_INCOMPLETE");
  });

  it("surfaces bounded replay and observability degradation as LIMITED", () => {
    const base = readinessInput();
    const replayLimited = sealStrategicReadiness({
      ...base,
      governanceReplay: { ...base.governanceReplay, result: { ...base.governanceReplay.result, replayState: "LIMITED" } },
    });
    const observabilityLimited = sealStrategicReadiness({
      ...base,
      audit: { ...base.audit, result: { ...base.audit.result, exportState: "LIMITED" } },
    });
    expect(replayLimited.result.readinessState).toBe("LIMITED");
    expect(replayLimited.validation.reasonCodes).toContain("REPLAY_DEGRADED");
    expect(observabilityLimited.result.readinessState).toBe("LIMITED");
  ...
  it("blocks cross-tenant access and ownership mismatch", () => {
    const base = readinessInput();
    const crossTenant = sealStrategicReadiness({
      ...base,
      replayEvidence: { ...base.replayEvidence, tenantId: "tenant-beta" },
    });
    const ownershipMismatch = sealStrategicReadiness({
      ...base,
      ownershipEvidence: { ...base.ownershipEvidence, recommendationId: "recommendation-other" },
    });
    expect(crossTenant.result.readinessState).toBe("NOT_READY");
    expect(crossTenant.validation.reasonCodes).toContain("CROSS_TENANT_READINESS_BLOCKED");
    expect(ownershipMismatch.validation.reasonCodes).toContain("OWNERSHIP_MISMATCH");
  });

  it("fails closed on governance or certification corruption", () => {
    const base = readinessInput();
    const governanceFailed = sealStrategicReadiness({
      ...base,
      governanceCertification: { ...base.governanceCertification, result: { ...base.governanceCertification.result, certificationState: "FAIL" } },
    });
    const replayCorrupted = sealStrategicReadiness({
      ...base,
      replay: { ...base.replay, result: { ...base.replay.result, replayState: "INVALID", replayIntegrity: false } },
    });
    expect(governanceFailed.result.readinessState).toBe("NOT_READY");
    expect(governanceFailed.validation.reasonCodes).toContain("CERTIFICATION_CORRUPTION_DETECTED");
  ...
  it("blocks mutation, execution, approval, ranking, workflow, governance execution, and authority expansion", () => {
    const base = readinessInput();
    expect(sealStrategicReadiness({ ...base, readinessMutationAttempted: true }).validation.reasonCodes).toContain("READINESS_MUTATION_DETECTED");
    expect(sealStrategicReadiness({ ...base, executionRequested: true }).validation.reasonCodes).toContain("EXECUTION_REQUEST_BLOCKED");
    expect(sealStrategicReadiness({ ...base, workflowRoutingRequested: true }).validation.reasonCodes).toContain("WORKFLOW_ROUTING_DETECTED");
    expect(sealStrategicReadiness({ ...base, recommendationApprovalRequested: true }).validation.reasonCodes).toContain("RECOMMENDATION_APPROVAL_DETECTED");
    expect(sealStrategicReadiness({ ...base, recommendationRankingRequested: true }).validation.reasonCodes).toContain("RECOMMENDATION_RANKING_DETECTED");
    expect(sealStrategicReadiness({ ...base, governanceExecutionRequested: true }).validation.reasonCodes).toContain("GOVERNANCE_EXECUTION_DETECTED");
    expect(sealStrategicReadiness({ ...base, authorityExpansionDetected: true }).validation.reasonCodes).toContain("AUTHORITY_EXPANSION_DETECTED");
  });

  it("does not mutate inputs", () => {
    const input = readinessInput();
    const before = JSON.stringify(input);
    sealStrategicReadiness(input);
