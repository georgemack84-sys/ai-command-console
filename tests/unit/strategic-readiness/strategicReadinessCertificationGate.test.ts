...
import {
  ...
  createStrategicReadinessCertificationEvidencePath,
...
function graphInput(overrides: Partial<DecisionGraphContractInput> = {}): DecisionGraphContractInput {
  const nodes: DecisionGraphNodeInput[] = [
    { nodeId: "boundary-node", graphId: "graph-510e", nodeType: "CONSTRAINT", tenantId: "tenant-alpha", lineageReference: "lineage-boundary-node" },
    { nodeId: "governance-anchor", graphId: "graph-510e", nodeType: "GOVERNANCE", tenantId: "tenant-alpha", lineageReference: "lineage-governance-anchor" },
    { nodeId: "recommendation-anchor", graphId: "graph-510e", nodeType: "RECOMMENDATION", tenantId: "tenant-alpha", lineageReference: "lineage-recommendation-anchor" },
    { nodeId: "escalation-anchor", graphId: "graph-510e", nodeType: "ESCALATION", tenantId: "tenant-alpha", lineageReference: "lineage-escalation-anchor" },
  ];
...
const proposalNodes = (): readonly ProposalNodeInput[] => [
  { proposalId: "proposal-a", graphId: "graph-510e", tenantId: "tenant-alpha", proposalType: "MISSION", lineageReference: "lineage-proposal-a" },
];
const governanceNodes = (): readonly GovernanceNodeInput[] => [
  { governanceId: "governance-a", graphId: "graph-510e", tenantId: "tenant-alpha", governanceType: "POLICY", lineageReference: "lineage-governance-a" },
];
const escalationNodes = (): readonly EscalationNodeInput[] => [
  { escalationId: "escalation-a", graphId: "graph-510e", tenantId: "tenant-alpha", escalationType: "REVIEW", lineageReference: "lineage-escalation-a" },
];
...
function buildCertificationInputs() {
  ...
  return { readiness, alignment, reviewPacket, replayFramework, ledger, lineage, verification, replay, integrity, certification, observability, inspection, visibility, audit, observabilityCertification, binding, authorityScope, policyVisibility, governanceReplay, governanceCertification, governanceReferences, ownershipEvidence, replayEvidence };
}

function certificationInput(overrides: Partial<StrategicReadinessCertificationInput> = {}): StrategicReadinessCertificationInput {
  const base = buildCertificationInputs();
  const request = buildStrategicReadinessCertificationRequest({ ...base, recommendationId: "recommendation-anchor", tenantId: "tenant-alpha", certificationScope: "FULL", graphVersion: "decision-graph/v1" });
  return Object.freeze({ request, ...base, ...overrides } satisfies StrategicReadinessCertificationInput);
}

describe("strategicReadinessCertificationGate", () => {
  it("is deterministic and reproduces certification hashes", () => {
    const input = certificationInput();
    const first = sealStrategicReadinessCertification(input);
    const second = sealStrategicReadinessCertification(input);
    expect(first).toEqual(second);
    expect(first.result.certificationState).toBe("PASS");
    expect(first.result.certificationHash).toHaveLength(64);
  });

  it("keeps certification ordering reproducible across scopes", () => {
    const base = certificationInput();
    const scopes = ["READINESS", "ALIGNMENT", "REVIEW_PACKET", "REPLAY", "FULL"] as const;
    for (const certificationScope of scopes) {
      const scoped = certificationInput({ request: { ...base.request, certificationScope } });
      expect(createStrategicReadinessCertificationEvidencePath(scoped)).toEqual(createStrategicReadinessCertificationEvidencePath(scoped));
      expect(sealStrategicReadinessCertification(scoped).result.certificationHash).toBe(sealStrategicReadinessCertification(scoped).result.certificationHash);
    }
  });

  it("produces CONDITIONAL_PASS for replay or observability degradation", () => {
    const base = certificationInput();
    const replayLimited = sealStrategicReadinessCertification({ ...base, replayFramework: { ...base.replayFramework, result: { ...base.replayFramework.result, replayState: "LIMITED" } } });
    const observabilityLimited = sealStrategicReadinessCertification({ ...base, audit: { ...base.audit, result: { ...base.audit.result, exportState: "LIMITED" } } });
  ...
  it("fails on broken readiness, alignment, packet, replay, or governance integrity", () => {
    const base = certificationInput();
    const brokenReadiness = sealStrategicReadinessCertification({ ...base, readiness: { ...base.readiness, result: { ...base.readiness.result, readinessState: "NOT_READY" } } });
    const brokenAlignment = sealStrategicReadinessCertification({ ...base, alignment: { ...base.alignment, result: { ...base.alignment.result, alignmentState: "MISALIGNED" } } });
    const brokenPacket = sealStrategicReadinessCertification({ ...base, reviewPacket: { ...base.reviewPacket, result: { ...base.reviewPacket.result, packetState: "INVALID" } } });
    const brokenReplay = sealStrategicReadinessCertification({ ...base, replayFramework: { ...base.replayFramework, result: { ...base.replayFramework.result, replayState: "ESCALATED" } } });
    const brokenGovernance = sealStrategicReadinessCertification({ ...base, governanceCertification: { ...base.governanceCertification, result: { ...base.governanceCertification.result, certificationState: "FAIL" } } });
    expect(brokenReadiness.result.certificationState).toBe("FAIL");
    expect(brokenReadiness.validation.reasonCodes).toContain("READINESS_INTEGRITY_BROKEN");
    expect(brokenAlignment.validation.reasonCodes).toContain("ALIGNMENT_INTEGRITY_BROKEN");
    expect(brokenPacket.validation.reasonCodes).toContain("REVIEW_PACKET_INTEGRITY_BROKEN");
  ...
  it("blocks cross-tenant certification and ownership mismatch", () => {
    const base = certificationInput();
    const crossTenant = sealStrategicReadinessCertification({ ...base, replayEvidence: { ...base.replayEvidence, tenantId: "tenant-beta" } });
    const ownershipMismatch = sealStrategicReadinessCertification({ ...base, ownershipEvidence: { ...base.ownershipEvidence, recommendationId: "recommendation-other" } });
  ...
  it("blocks mutation, execution, approval, ranking, workflow, governance execution, and authority expansion", () => {
    const base = certificationInput();
    expect(sealStrategicReadinessCertification({ ...base, certificationMutationAttempted: true }).validation.reasonCodes).toContain("CERTIFICATION_MUTATION_DETECTED");
    expect(sealStrategicReadinessCertification({ ...base, executionRequested: true }).validation.reasonCodes).toContain("EXECUTION_REQUEST_BLOCKED");
    expect(sealStrategicReadinessCertification({ ...base, workflowRoutingRequested: true }).validation.reasonCodes).toContain("WORKFLOW_ROUTING_DETECTED");
    expect(sealStrategicReadinessCertification({ ...base, recommendationApprovalRequested: true }).validation.reasonCodes).toContain("RECOMMENDATION_APPROVAL_DETECTED");
    expect(sealStrategicReadinessCertification({ ...base, recommendationRankingRequested: true }).validation.reasonCodes).toContain("RECOMMENDATION_RANKING_DETECTED");
    expect(sealStrategicReadinessCertification({ ...base, governanceExecutionRequested: true }).validation.reasonCodes).toContain("GOVERNANCE_EXECUTION_DETECTED");
    expect(sealStrategicReadinessCertification({ ...base, authorityExpansionDetected: true }).validation.reasonCodes).toContain("AUTHORITY_EXPANSION_DETECTED");
  });

  it("does not mutate inputs", () => {
    const input = certificationInput();
    const before = JSON.stringify(input);
    sealStrategicReadinessCertification(input);
