...
function graphInput(overrides: Partial<DecisionGraphContractInput> = {}): DecisionGraphContractInput {
  const nodes: DecisionGraphNodeInput[] = [
    { nodeId: "boundary-node", graphId: "graph-510d", nodeType: "CONSTRAINT", tenantId: "tenant-alpha", lineageReference: "lineage-boundary-node" },
    { nodeId: "governance-anchor", graphId: "graph-510d", nodeType: "GOVERNANCE", tenantId: "tenant-alpha", lineageReference: "lineage-governance-anchor" },
    { nodeId: "recommendation-anchor", graphId: "graph-510d", nodeType: "RECOMMENDATION", tenantId: "tenant-alpha", lineageReference: "lineage-recommendation-anchor" },
    { nodeId: "escalation-anchor", graphId: "graph-510d", nodeType: "ESCALATION", tenantId: "tenant-alpha", lineageReference: "lineage-escalation-anchor" },
  ];
...
const proposalNodes = (): readonly ProposalNodeInput[] => [
  { proposalId: "proposal-a", graphId: "graph-510d", tenantId: "tenant-alpha", proposalType: "MISSION", lineageReference: "lineage-proposal-a" },
];
const governanceNodes = (): readonly GovernanceNodeInput[] => [
  { governanceId: "governance-a", graphId: "graph-510d", tenantId: "tenant-alpha", governanceType: "POLICY", lineageReference: "lineage-governance-a" },
];
const escalationNodes = (): readonly EscalationNodeInput[] => [
  { escalationId: "escalation-a", graphId: "graph-510d", tenantId: "tenant-alpha", escalationType: "REVIEW", lineageReference: "lineage-escalation-a" },
];
...
function buildReplayInputs() {
  ...
  return { readiness, alignment, reviewPacket, ledger, lineage, verification, replay, integrity, certification, observability, inspection, visibility, audit, observabilityCertification, binding, authorityScope, policyVisibility, governanceReplay, governanceCertification, governanceReferences, ownershipEvidence, replayEvidence };
}

function replayInput(overrides: Partial<StrategicReadinessReplayInput> = {}): StrategicReadinessReplayInput {
  const base = buildReplayInputs();
  const request = buildStrategicReadinessReplayRequest({ ...base, recommendationId: "recommendation-anchor", tenantId: "tenant-alpha", replayScope: "FULL", replayVersion: "strategic-readiness-replay/v1", graphVersion: "decision-graph/v1" });
  return Object.freeze({ request, ...base, ...overrides } satisfies StrategicReadinessReplayInput);
}

describe("strategicReadinessReplayFramework", () => {
  it("is deterministic and reproduces replay and reconstruction hashes", () => {
    const input = replayInput();
    const first = sealStrategicReadinessReplay(input);
    const second = sealStrategicReadinessReplay(input);
  ...
  it("keeps reconstruction ordering reproducible across scopes", () => {
    const base = replayInput();
    const scopes = ["READINESS", "ALIGNMENT", "REVIEW_PACKET", "FULL"] as const;
    for (const replayScope of scopes) {
      const scoped = replayInput({ request: { ...base.request, replayScope } });
      expect(createStrategicReadinessReplayEvidencePath(scoped)).toEqual(createStrategicReadinessReplayEvidencePath(scoped));
      expect(sealStrategicReadinessReplay(scoped).result.replayHash).toBe(sealStrategicReadinessReplay(scoped).result.replayHash);
    }
  });

  it("surfaces missing readiness, alignment, or review packet evidence as LIMITED", () => {
    const base = replayInput();
    const missingReadiness = sealStrategicReadinessReplay({ ...base, readiness: { ...base.readiness, result: { ...base.readiness.result, readinessState: "NOT_READY" } } });
    const missingAlignment = sealStrategicReadinessReplay({ ...base, alignment: { ...base.alignment, result: { ...base.alignment.result, alignmentState: "MISALIGNED" } } });
    const missingPacket = sealStrategicReadinessReplay({ ...base, reviewPacket: { ...base.reviewPacket, result: { ...base.reviewPacket.result, packetState: "INVALID" } } });
    expect(missingReadiness.result.replayState).toBe("LIMITED");
    expect(missingReadiness.validation.reasonCodes).toContain("READINESS_RECONSTRUCTION_MISSING");
    expect(missingAlignment.validation.reasonCodes).toContain("ALIGNMENT_RECONSTRUCTION_MISSING");
    expect(missingPacket.validation.reasonCodes).toContain("REVIEW_PACKET_RECONSTRUCTION_MISSING");
  });

  it("escalates on governance reconstruction degradation or replay hash mismatch", () => {
    const base = replayInput();
    const governanceBroken = sealStrategicReadinessReplay({ ...base, governanceCertification: { ...base.governanceCertification, result: { ...base.governanceCertification.result, certificationState: "FAIL" } } });
    const hashMismatch = sealStrategicReadinessReplay({ ...base, replay: { ...base.replay, result: { ...base.replay.result, replayHash: "x".repeat(10) } } });
    expect(governanceBroken.result.replayState).toBe("ESCALATED");
    expect(governanceBroken.validation.reasonCodes).toContain("GOVERNANCE_RECONSTRUCTION_DEGRADED");
  ...
  it("blocks cross-tenant replay and ownership mismatch", () => {
    const base = replayInput();
    const crossTenant = sealStrategicReadinessReplay({ ...base, replayEvidence: { ...base.replayEvidence, tenantId: "tenant-beta" } });
    const ownershipMismatch = sealStrategicReadinessReplay({ ...base, ownershipEvidence: { ...base.ownershipEvidence, recommendationId: "recommendation-other" } });
  ...
  it("blocks mutation, execution, approval, ranking, workflow, governance execution, and authority expansion", () => {
    const base = replayInput();
    expect(sealStrategicReadinessReplay({ ...base, replayMutationAttempted: true }).validation.reasonCodes).toContain("REPLAY_MUTATION_DETECTED");
    expect(sealStrategicReadinessReplay({ ...base, executionRequested: true }).validation.reasonCodes).toContain("EXECUTION_REQUEST_BLOCKED");
    expect(sealStrategicReadinessReplay({ ...base, workflowRoutingRequested: true }).validation.reasonCodes).toContain("WORKFLOW_ROUTING_DETECTED");
    expect(sealStrategicReadinessReplay({ ...base, recommendationApprovalRequested: true }).validation.reasonCodes).toContain("RECOMMENDATION_APPROVAL_DETECTED");
    expect(sealStrategicReadinessReplay({ ...base, recommendationRankingRequested: true }).validation.reasonCodes).toContain("RECOMMENDATION_RANKING_DETECTED");
    expect(sealStrategicReadinessReplay({ ...base, governanceExecutionRequested: true }).validation.reasonCodes).toContain("GOVERNANCE_EXECUTION_DETECTED");
    expect(sealStrategicReadinessReplay({ ...base, authorityExpansionDetected: true }).validation.reasonCodes).toContain("AUTHORITY_EXPANSION_DETECTED");
  });

  it("does not mutate inputs", () => {
    const input = replayInput();
    const before = JSON.stringify(input);
    sealStrategicReadinessReplay(input);
