...
import {
  ...
  createOpportunityReplayEvidencePath,
...
describe("opportunityReplayFramework", () => {
  it("is deterministic and reproduces replay hashes", () => {
    const input = opportunityReplayInput();
    const first = sealOpportunityReplay(input);
    const second = sealOpportunityReplay(input);
  ...
  it("keeps replay ordering reproducible", () => {
    const input = opportunityReplayInput();
    const reversed = opportunityReplayInput({
      recommendations: Object.freeze([...input.recommendations].reverse()),
    });
    expect(sealOpportunityReplay(reversed)).toEqual(sealOpportunityReplay(input));
    const sealed = sealOpportunityReplay(input);
    expect(createOpportunityReplayEvidencePath(input)).toEqual(createOpportunityReplayEvidencePath(input));
    expect(sealed.evidencePath.propagationReferences).toEqual([...sealed.evidencePath.propagationReferences].sort());
  }, 15000);

  it("preserves opportunity, strength, propagation, conflicts, and governance reconstruction", () => {
    const sealed = sealOpportunityReplay(opportunityReplayInput());
    expect(sealed.result.opportunityReconstructed).toBe(true);
    expect(sealed.result.strengthReconstructed).toBe(true);
  ...
  it("surfaces missing replay artifacts as LIMITED", () => {
    const base = opportunityReplayInput();
    const limited = sealOpportunityReplay({
      ...base,
  ...
  it("surfaces replay mismatches and lineage breaks as ESCALATED", () => {
    const base = opportunityReplayInput();
    const hashMismatch = sealOpportunityReplay({
      ...base,
      recommendations: Object.freeze([
        {
          ...base.recommendations[0],
          replay: {
            ...base.recommendations[0].replay,
            result: { ...base.recommendations[0].replay.result, replayHash: "bad-hash" },
          },
        },
        ...base.recommendations.slice(1),
      ]),
    });
    const lineageBreak = sealOpportunityReplay({
      ...base,
      recommendations: Object.freeze([
        {
          ...base.recommendations[0],
          lineage: {
            ...
            result: { ...base.recommendations[0].lineage.result, lineageIntegrity: false },
          },
        },
        ...base.recommendations.slice(1),
      ]),
    });
    expect(hashMismatch.result.replayState).toBe("ESCALATED");
    expect(hashMismatch.validation.reasonCodes).toContain("REPLAY_HASH_MISMATCH");
    expect(lineageBreak.result.replayState).toBe("ESCALATED");
    expect(lineageBreak.validation.reasonCodes).toContain("LINEAGE_CONTINUITY_BROKEN");
  });

  it("blocks cross-tenant replay and ownership mismatch", () => {
    const base = opportunityReplayInput();
    const crossTenant = sealOpportunityReplay({
      ...base,
      recommendations: Object.freeze([
    ...
    const ownershipMismatch = sealOpportunityReplay({
      ...base,
      recommendations: Object.freeze([
  ...
  it("fails on governance corruption", () => {
    const base = opportunityReplayInput();
    const brokenGovernance = sealOpportunityReplay({
      ...base,
      recommendations: Object.freeze([
  ...
  it("blocks execution, mutation, workflow routing, prioritization, ranking, approval, resource allocation, and authority expansion", () => {
    const base = opportunityReplayInput();
    expect(sealOpportunityReplay({ ...base, executionRequested: true }).validation.reasonCodes).toContain("EXECUTION_REQUEST_BLOCKED");
    expect(sealOpportunityReplay({ ...base, replayMutationAttempted: true }).validation.reasonCodes).toContain("REPLAY_MUTATION_DETECTED");
    expect(sealOpportunityReplay({ ...base, workflowRoutingRequested: true }).validation.reasonCodes).toContain("WORKFLOW_ROUTING_DETECTED");
    expect(sealOpportunityReplay({ ...base, prioritizationRequested: true }).validation.reasonCodes).toContain("PRIORITIZATION_DETECTED");
    expect(sealOpportunityReplay({ ...base, recommendationRankingRequested: true }).validation.reasonCodes).toContain("RANKING_DETECTED");
    expect(sealOpportunityReplay({ ...base, approvalRequested: true }).validation.reasonCodes).toContain("APPROVAL_DETECTED");
    expect(sealOpportunityReplay({ ...base, resourceAllocationRequested: true }).validation.reasonCodes).toContain("RESOURCE_ALLOCATION_DETECTED");
    expect(sealOpportunityReplay({ ...base, authorityExpansionDetected: true }).validation.reasonCodes).toContain("AUTHORITY_EXPANSION_DETECTED");
  });
});
