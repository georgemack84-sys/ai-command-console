...
import {
  ...
  createOpportunityAnalysisEvidencePath,
...
describe("opportunityAnalysisEngine", () => {
  it("is deterministic and reproduces analysis hashes", () => {
      const input = opportunityAnalysisInput();
      const first = sealOpportunityAnalysis(input);
      const second = sealOpportunityAnalysis(input);
  ...
  it("keeps analysis ordering deterministic", () => {
      const input = opportunityAnalysisInput();
      const reversed = opportunityAnalysisInput({
        recommendations: Object.freeze([...input.recommendations].reverse()),
      });
      const first = sealOpportunityAnalysis(input);
      const second = sealOpportunityAnalysis(reversed);
  ...
  it("reproduces strong, moderate, weak, constrained, and unsupported strengths", () => {
      const source = recommendationOpportunityInput();
      const base = opportunityAnalysisInput();
      const strong = sealOpportunityAnalysis(base);
      const moderate = sealOpportunityAnalysis({
        ...base,
        trustFoundation: {
          ...base.trustFoundation,
          result: {
            ...base.trustFoundation.result,
            trustState: "DEGRADED" as const,
          },
        },
      });
      const weakFoundation = sealRecommendationOpportunityFoundation({
        ...source,
        recommendations: Object.freeze([
          {
            ...source.recommendations[0],
            readiness: {
              ...source.recommendations[0].readiness,
              result: {
                ...source.recommendations[0].readiness.result,
                readinessState: "LIMITED" as const,
              },
            },
          },
          ...source.recommendations.slice(1),
        ]),
      });
      const weak = sealOpportunityAnalysis({ ...base, foundation: weakFoundation });
      const constrainedFoundation = sealRecommendationOpportunityFoundation({
        ...source,
        trustCertification: {
          ...source.trustCertification,
          result: {
            ...source.trustCertification.result,
            certificationState: "CONDITIONAL_PASS" as const,
          },
        },
      });
      const constrained = sealOpportunityAnalysis({ ...base, foundation: constrainedFoundation });
      const unsupportedFoundation = sealRecommendationOpportunityFoundation({
        ...source,
        dependencyRiskCertification: {
          ...source.dependencyRiskCertification,
          result: {
            ...source.dependencyRiskCertification.result,
            governanceCertified: false,
            certificationState: "FAIL" as const,
          },
        },
      });
      const unsupported = sealOpportunityAnalysis({
        ...base,
        foundation: unsupportedFoundation,
        dependencyRiskCertification: {
      ...
      expect(strong.strengths.some((strength) => strength.strength === "STRONG")).toBe(true);
      expect(moderate.strengths.some((strength) => strength.strength === "MODERATE")).toBe(true);
      expect(weak.strengths.some((strength) => strength.strength === "WEAK")).toBe(true);
      expect(constrained.strengths.some((strength) => strength.strength === "CONSTRAINED")).toBe(true);
      expect(unsupported.strengths.some((strength) => strength.strength === "UNSUPPORTED")).toBe(true);
  });

  it("surfaces concentrations and propagations reproducibly", () => {
      const sealed = sealOpportunityAnalysis(opportunityAnalysisInput());
      expect(sealed.result.opportunityConcentrationsDetected).toBeGreaterThan(0);
      expect(sealed.result.opportunityPropagationsDetected).toBeGreaterThan(0);
      expect(sealed.evidencePath.concentrationReferences.length).toBe(sealed.result.opportunityConcentrationsDetected);
      expect(sealed.evidencePath.propagationReferences.length).toBe(sealed.result.opportunityPropagationsDetected);
  });

  it("surfaces gaps and conflicts as LIMITED", () => {
      const source = recommendationOpportunityInput();
      const base = opportunityAnalysisInput();
      const limited = sealOpportunityAnalysis({
        ...base,
        trustFoundation: {
          ...base.trustFoundation,
          result: {
        ...
        foundation: sealRecommendationOpportunityFoundation({
          ...source,
          recommendations: Object.freeze([
            {
              ...source.recommendations[0],
              replayEvidence: {
                ...source.recommendations[0].replayEvidence,
                replayReferences: [],
              },
            },
            ...source.recommendations.slice(1),
      ...
      expect(limited.validation.reasonCodes).toContain("OPPORTUNITY_GAPS_DETECTED");
  });

  it("surfaces missing opportunity evidence as OBSERVE", () => {
      const source = recommendationOpportunityInput();
      const foundation = sealRecommendationOpportunityFoundation({
        ...source,
        request: buildRecommendationOpportunityFoundationRequest({
          tenantId: "tenant-alpha",
          recommendationIds: ["recommendation-alpha", "recommendation-beta", "recommendation-gamma"],
          opportunityScope: "FULL",
          graphVersion: "decision-graph/v1",
        }),
      });
      const observe = sealOpportunityAnalysis(opportunityAnalysisInput({
        foundation,
        request: buildOpportunityAnalysisRequest({
          tenantId: "tenant-alpha",
          recommendationIds: ["recommendation-alpha", "recommendation-beta", "recommendation-gamma"],
          analysisScope: "FULL",
          graphVersion: "decision-graph/v1",
  ...
  it("blocks cross-tenant opportunity analysis and ownership mismatches", () => {
      const base = opportunityAnalysisInput();
      const crossTenant = sealOpportunityAnalysis({
        ...base,
        recommendations: Object.freeze([
          {
      ...
      const ownershipMismatch = sealOpportunityAnalysis({
        ...base,
        recommendations: Object.freeze([
          {
            ...base.recommendations[0],
  ...
  it("blocks execution, approval, ranking, prioritization, scoring, resource allocation, mutation, and authority expansion", () => {
      const base = opportunityAnalysisInput();

      expect(sealOpportunityAnalysis({ ...base, executionRequested: true }).validation.reasonCodes).toContain("EXECUTION_REQUEST_BLOCKED");
      expect(sealOpportunityAnalysis({ ...base, approvalRequested: true }).validation.reasonCodes).toContain("APPROVAL_DETECTED");
      expect(sealOpportunityAnalysis({ ...base, recommendationRankingRequested: true }).validation.reasonCodes).toContain("RANKING_DETECTED");
      expect(sealOpportunityAnalysis({ ...base, prioritizationRequested: true }).validation.reasonCodes).toContain("PRIORITIZATION_DETECTED");
      expect(sealOpportunityAnalysis({ ...base, recommendationScoringRequested: true }).validation.reasonCodes).toContain("SCORING_DETECTED");
      expect(sealOpportunityAnalysis({ ...base, resourceAllocationRequested: true }).validation.reasonCodes).toContain("RESOURCE_ALLOCATION_DETECTED");
      expect(sealOpportunityAnalysis({ ...base, analysisMutationAttempted: true }).validation.reasonCodes).toContain("ANALYSIS_MUTATION_DETECTED");
      expect(sealOpportunityAnalysis({ ...base, authorityExpansionDetected: true }).validation.reasonCodes).toContain("AUTHORITY_EXPANSION_DETECTED");
  });
});
