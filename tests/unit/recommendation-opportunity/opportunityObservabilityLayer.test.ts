...
import {
  ...
  createOpportunityObservabilityEvidencePath,
...
describe("opportunityObservabilityLayer", () => {
  it("is deterministic and reproduces observability hashes", () => {
    const input = opportunityObservabilityInput();
    const first = sealOpportunityObservability(input);
    const second = sealOpportunityObservability(input);
  ...
  it("keeps visibility ordering reproducible", () => {
    const input = opportunityObservabilityInput();
    const reversed = opportunityObservabilityInput({
      recommendations: Object.freeze([...input.recommendations].reverse()),
    });
    expect(sealOpportunityObservability(reversed)).toEqual(sealOpportunityObservability(input));
    const sealed = sealOpportunityObservability(input);
    expect(createOpportunityObservabilityEvidencePath(input)).toEqual(createOpportunityObservabilityEvidencePath(input));
    expect(sealed.evidencePath.propagationReferences).toEqual([...sealed.evidencePath.propagationReferences].sort());
  });

  it("preserves graph, strength, propagation, lineage, governance, replay, and audit visibility", () => {
    const sealed = sealOpportunityObservability(opportunityObservabilityInput());
    expect(sealed.result.opportunityGraphVisible).toBe(true);
    expect(sealed.result.opportunityStrengthVisible).toBe(true);
    expect(sealed.result.opportunityPropagationVisible).toBe(true);
    expect(sealed.result.opportunityLineageVisible).toBe(true);
    expect(sealed.result.opportunityGovernanceVisible).toBe(true);
    expect(sealed.result.opportunityReplayVisible).toBe(true);
    expect(sealed.result.opportunityAuditVisible).toBe(true);
  });

  it("surfaces replay visibility degradation as LIMITED", () => {
    const base = opportunityObservabilityInput();
    const limited = sealOpportunityObservability({
      ...base,
      recommendations: Object.freeze([
        {
          ...base.recommendations[0],
          replayEvidence: { ...base.recommendations[0].replayEvidence, replayReferences: Object.freeze([]) },
          replay: {
    ...
    expect(limited.validation.reasonCodes).toContain("OPPORTUNITY_REPLAY_VISIBILITY_MISSING");
  });

  it("surfaces missing visibility evidence as OBSERVE", () => {
    const base = opportunityObservabilityInput();
    const observe = sealOpportunityObservability({
      ...base,
      foundation: {
        ...base.foundation,
  ...
  it("blocks cross-tenant visibility and ownership mismatch", () => {
    const base = opportunityObservabilityInput();
    const crossTenant = sealOpportunityObservability({
      ...base,
      recommendations: Object.freeze([
    ...
    const ownershipMismatch = sealOpportunityObservability({
      ...base,
      recommendations: Object.freeze([
  ...
  it("fails on governance corruption", () => {
    const base = opportunityObservabilityInput();
    const brokenGovernance = sealOpportunityObservability({
      ...base,
      recommendations: Object.freeze([
  ...
  it("blocks execution, mutation, workflow routing, prioritization, ranking, approval, resource allocation, and authority expansion", () => {
    const base = opportunityObservabilityInput();
    expect(sealOpportunityObservability({ ...base, executionRequested: true }).validation.reasonCodes).toContain("EXECUTION_REQUEST_BLOCKED");
    expect(sealOpportunityObservability({ ...base, observabilityMutationAttempted: true }).validation.reasonCodes).toContain("OBSERVABILITY_MUTATION_DETECTED");
    expect(sealOpportunityObservability({ ...base, workflowRoutingRequested: true }).validation.reasonCodes).toContain("WORKFLOW_ROUTING_DETECTED");
    expect(sealOpportunityObservability({ ...base, prioritizationRequested: true }).validation.reasonCodes).toContain("PRIORITIZATION_DETECTED");
    expect(sealOpportunityObservability({ ...base, recommendationRankingRequested: true }).validation.reasonCodes).toContain("RANKING_DETECTED");
    expect(sealOpportunityObservability({ ...base, approvalRequested: true }).validation.reasonCodes).toContain("APPROVAL_DETECTED");
    expect(sealOpportunityObservability({ ...base, resourceAllocationRequested: true }).validation.reasonCodes).toContain("RESOURCE_ALLOCATION_DETECTED");
    expect(sealOpportunityObservability({ ...base, authorityExpansionDetected: true }).validation.reasonCodes).toContain("AUTHORITY_EXPANSION_DETECTED");
  });
});
