...
import {
  ...
  createRecommendationOpportunityEvidencePath,
...
describe("recommendationOpportunityFoundation", () => {
  it("is deterministic and produces stable opportunity hashes", () => {
    const input = recommendationOpportunityInput();
    const first = sealRecommendationOpportunityFoundation(input);
    const second = sealRecommendationOpportunityFoundation(input);

    expect(first).toEqual(second);
    expect(first.result.opportunityGraphHash).toHaveLength(64);
  });

  it("keeps opportunity ordering deterministic when recommendations are reversed", () => {
    const input = recommendationOpportunityInput();
    const reversedRecommendations = Object.freeze([...input.recommendations].reverse());
    const reversed = recommendationOpportunityInput({
      recommendations: reversedRecommendations,
      request: input.request,
    });

    const first = sealRecommendationOpportunityFoundation(input);
    const second = sealRecommendationOpportunityFoundation(reversed);

    expect(second.opportunities).toEqual(first.opportunities);
    expect(second.evidencePath).toEqual(first.evidencePath);
    expect(second.result.opportunityGraphHash).toBe(first.result.opportunityGraphHash);
    expect(
      createRecommendationOpportunityEvidencePath(reversed, second.opportunities),
    ).toEqual(createRecommendationOpportunityEvidencePath(input, first.opportunities));
  });

  it("builds all seven opportunity types reproducibly", () => {
    const input = recommendationOpportunityInput();
    const sealed = sealRecommendationOpportunityFoundation(input);

    expect(sealed.result.opportunityState).toBe("SUPPORTED");
    expect(sealed.result.opportunitiesCreated).toBe(input.request.recommendationIds.length * 7);
    expect(
      new Set(sealed.opportunities.map((opportunity) => opportunity.opportunityType)),
    ).toEqual(new Set([
    ...
    expect(sealed.result.governanceOpportunitiesDetected).toBeGreaterThan(0);
    expect(sealed.result.dependencyOpportunitiesDetected).toBeGreaterThan(0);
    expect(sealed.result.impactOpportunitiesDetected).toBeGreaterThan(0);
    expect(sealed.result.portfolioOpportunitiesDetected).toBeGreaterThan(0);
    expect(sealed.result.trustOpportunitiesDetected).toBeGreaterThan(0);
    expect(sealed.result.resilienceOpportunitiesDetected).toBeGreaterThan(0);
    expect(sealed.result.readinessOpportunitiesDetected).toBeGreaterThan(0);
  });

  it("surfaces supported, conditional, limited, unsupported, and unknown states", () => {
    const base = recommendationOpportunityInput();
    const supported = sealRecommendationOpportunityFoundation(base);
    const conditional = sealRecommendationOpportunityFoundation({
      ...base,
      trustCertification: {
        ...base.trustCertification,
        result: {
          ...base.trustCertification.result,
          certificationState: "CONDITIONAL_PASS" as const,
        },
      },
    });
    const limited = sealRecommendationOpportunityFoundation({
      ...base,
      recommendations: Object.freeze([
        {
          ...
          readiness: {
            ...
            result: {
              ...base.recommendations[0].readiness.result,
              readinessState: "LIMITED" as const,
            },
          },
        },
    ...
    const unsupported = sealRecommendationOpportunityFoundation({
      ...base,
      dependencyRiskCertification: {
    ...
    const unknown = sealRecommendationOpportunityFoundation({
      ...base,
      recommendations: Object.freeze([
        {
          ...base.recommendations[0],
    ...
    expect(supported.result.opportunityState).toBe("SUPPORTED");
    expect(conditional.result.opportunityState).toBe("CONDITIONALLY_SUPPORTED");
    expect(conditional.validation.reasonCodes).toContain("TRUST_OPPORTUNITY_CONDITIONAL");
    expect(limited.result.opportunityState).toBe("LIMITED");
    expect(limited.validation.reasonCodes).toContain("READINESS_OPPORTUNITY_LIMITED");
    expect(unsupported.result.opportunityState).toBe("UNSUPPORTED");
    expect(unsupported.validation.reasonCodes).toContain("GOVERNANCE_CORRUPTION_DETECTED");
    expect(unknown.result.opportunityState).toBe("UNKNOWN");
  ...
  it("blocks cross-tenant inputs and ownership mismatches", () => {
    const base = recommendationOpportunityInput();
    const crossTenant = sealRecommendationOpportunityFoundation({
      ...base,
      dependencyRiskFoundation: {
    ...
    const ownershipMismatch = sealRecommendationOpportunityFoundation({
      ...base,
      recommendations: Object.freeze([
        {
          ...base.recommendations[0],
    ...
    expect(crossTenant.result.opportunityState).toBe("UNSUPPORTED");
    expect(crossTenant.validation.reasonCodes).toContain("CROSS_TENANT_OPPORTUNITY_BLOCKED");
    expect(ownershipMismatch.result.opportunityState).toBe("UNSUPPORTED");
  ...
  it("blocks execution, mutation, routing, prioritization, ranking, approval, scoring, resource allocation, and authority expansion", () => {
    ...
    expect(
      sealRecommendationOpportunityFoundation({ ...base, executionRequested: true }).validation.reasonCodes,
    ).toContain("EXECUTION_REQUEST_BLOCKED");
    expect(
      sealRecommendationOpportunityFoundation({ ...base, opportunityMutationAttempted: true }).validation.reasonCodes,
    ).toContain("OPPORTUNITY_MUTATION_DETECTED");
    expect(
      sealRecommendationOpportunityFoundation({ ...base, workflowRoutingRequested: true }).validation.reasonCodes,
    ).toContain("WORKFLOW_ROUTING_DETECTED");
    expect(
      sealRecommendationOpportunityFoundation({ ...base, prioritizationRequested: true }).validation.reasonCodes,
    ).toContain("PRIORITIZATION_DETECTED");
    expect(
      sealRecommendationOpportunityFoundation({ ...base, recommendationRankingRequested: true }).validation.reasonCodes,
    ).toContain("RANKING_DETECTED");
    expect(
      sealRecommendationOpportunityFoundation({ ...base, approvalRequested: true }).validation.reasonCodes,
    ).toContain("APPROVAL_DETECTED");
    expect(
      sealRecommendationOpportunityFoundation({ ...base, recommendationScoringRequested: true }).validation.reasonCodes,
    ).toContain("SCORING_DETECTED");
    expect(
      sealRecommendationOpportunityFoundation({ ...base, resourceAllocationRequested: true }).validation.reasonCodes,
    ).toContain("RESOURCE_ALLOCATION_DETECTED");
    expect(
      sealRecommendationOpportunityFoundation({ ...base, authorityExpansionDetected: true }).validation.reasonCodes,
  ...
  it("bounds governance references at the declared limit", () => {
    ...
    const failed = sealRecommendationOpportunityFoundation({
      ...base,
      recommendations: Object.freeze([
        {
          ...base.recommendations[0],
    ...
    expect(failed.result.opportunityState).toBe("UNSUPPORTED");
