...
import {
  ...
  createDependencyHealthAnalysisEvidencePath,
...
describe("dependencyHealthAnalysisEngine", () => {
  it("is deterministic and produces a stable analysis hash", () => {
    const input = dependencyHealthAnalysisInput();
    const first = sealDependencyHealthAnalysis(input);
    const second = sealDependencyHealthAnalysis(input);
  ...
  it("keeps analysis ordering deterministic when recommendations are reversed", () => {
    const base = dependencyHealthAnalysisInput();
    const reordered = dependencyHealthAnalysisInput({
      recommendations: Object.freeze([...base.recommendations].reverse()),
    });
    const sealed = sealDependencyHealthAnalysis(reordered);

    expect(sealed).toEqual(sealDependencyHealthAnalysis(reordered));
    expect(createDependencyHealthAnalysisEvidencePath(
      reordered,
      sealed.stabilityConditions,
      sealed.availabilityConditions,
      sealed.continuityConditions,
      sealed.recoverabilityConditions,
      sealed.degradationConditions,
      sealed.riskConditions,
      sealed.observabilityConditions,
    )).toEqual(sealed.evidencePath);
  }, 90000);

  it("builds all seven dependency health analysis domains reproducibly", () => {
    const sealed = sealDependencyHealthAnalysis(dependencyHealthAnalysisInput());

    expect(sealed.result.stabilityConditionsDetected).toBeGreaterThan(0);
    expect(sealed.result.availabilityConditionsDetected).toBeGreaterThan(0);
    expect(sealed.result.continuityConditionsDetected).toBeGreaterThan(0);
    expect(sealed.result.recoverabilityConditionsDetected).toBeGreaterThan(0);
    expect(sealed.result.degradationConditionsDetected).toBeGreaterThan(0);
    expect(sealed.result.riskConditionsDetected).toBeGreaterThan(0);
    expect(sealed.result.observabilityConditionsDetected).toBeGreaterThan(0);
  }, 30000);

  it("surfaces ANALYZED, LIMITED, OBSERVE, and INVALID states", () => {
    const base = dependencyHealthAnalysisInput();
    const limitedBase = dependencyHealthFoundationInput();
    const observeBase = dependencyHealthFoundationInput();
    const analyzed = sealDependencyHealthAnalysis(base);
    const limited = sealDependencyHealthAnalysis(dependencyHealthAnalysisInput({
      foundation: sealRecommendationDependencyHealthFoundation({
        ...limitedBase,
        dependencyFoundation: {
          ...limitedBase.dependencyFoundation,
          result: {
            ...limitedBase.dependencyFoundation.result,
            dependencyState: "LIMITED",
          },
        },
      }),
    }));
    const observe = sealDependencyHealthAnalysis(dependencyHealthAnalysisInput({
      foundation: sealRecommendationDependencyHealthFoundation({
        ...observeBase,
        recommendations: Object.freeze([
          {
            ...observeBase.recommendations[0],
            governanceReferences: {
              ...observeBase.recommendations[0].governanceReferences,
              governanceReferences: [],
            },
            binding: {
              ...observeBase.recommendations[0].binding,
              evidencePath: {
                ...observeBase.recommendations[0].binding.evidencePath,
                governanceReferences: [],
              },
            },
            authorityScope: {
              ...observeBase.recommendations[0].authorityScope,
              evidencePath: {
                ...observeBase.recommendations[0].authorityScope.evidencePath,
                governanceReferences: [],
              },
            },
            policyVisibility: {
              ...observeBase.recommendations[0].policyVisibility,
              evidencePath: {
                ...observeBase.recommendations[0].policyVisibility.evidencePath,
                governanceReferences: [],
              },
            },
            governanceReplay: {
              ...observeBase.recommendations[0].governanceReplay,
              evidencePath: {
                ...observeBase.recommendations[0].governanceReplay.evidencePath,
                governanceReferences: [],
              },
            },
            governanceCertification: {
              ...observeBase.recommendations[0].governanceCertification,
              evidencePath: {
                ...observeBase.recommendations[0].governanceCertification.evidencePath,
                governanceReferences: [],
              },
            },
            readiness: {
              ...observeBase.recommendations[0].readiness,
              evidencePath: {
                ...observeBase.recommendations[0].readiness.evidencePath,
                governanceReferences: [],
              },
            },
            alignment: {
              ...observeBase.recommendations[0].alignment,
              evidencePath: {
                ...observeBase.recommendations[0].alignment.evidencePath,
                governanceReferences: [],
              },
            },
          },
          ...observeBase.recommendations.slice(1),
        ]),
      }),
    }));
    const invalid = sealDependencyHealthAnalysis({
      ...base,
      authorityExpansionDetected: true,
    });

    expect(analyzed.result.analysisState).toBe("ANALYZED");
    expect(limited.result.analysisState).toBe("LIMITED");
    expect(observe.result.analysisState).toBe("OBSERVE");
    expect(invalid.result.analysisState).toBe("INVALID");
  }, 180000);

  it("blocks cross-tenant health analysis, ownership mismatches, execution, mutation, routing, prioritization, ranking, approval, scoring, resource allocation, and authority expansion", () => {
    const base = dependencyHealthAnalysisInput();
    const crossTenant = sealDependencyHealthAnalysis({
      ...base,
      dependencyRiskFoundation: {
    ...
    const ownershipMismatch = sealDependencyHealthAnalysis({
      ...base,
      recommendations: Object.freeze([
        {
    ...
    expect(crossTenant.validation.reasonCodes).toContain("CROSS_TENANT_HEALTH_ANALYSIS_BLOCKED");
    expect(ownershipMismatch.validation.reasonCodes).toContain("OWNERSHIP_MISMATCH");
    expect(sealDependencyHealthAnalysis({ ...base, executionRequested: true }).validation.reasonCodes).toContain("EXECUTION_REQUEST_BLOCKED");
    expect(sealDependencyHealthAnalysis({ ...base, analysisMutationAttempted: true }).validation.reasonCodes).toContain("ANALYSIS_MUTATION_DETECTED");
    expect(sealDependencyHealthAnalysis({ ...base, workflowRoutingRequested: true }).validation.reasonCodes).toContain("WORKFLOW_ROUTING_DETECTED");
    expect(sealDependencyHealthAnalysis({ ...base, prioritizationRequested: true }).validation.reasonCodes).toContain("PRIORITIZATION_DETECTED");
    expect(sealDependencyHealthAnalysis({ ...base, recommendationRankingRequested: true }).validation.reasonCodes).toContain("RANKING_DETECTED");
    expect(sealDependencyHealthAnalysis({ ...base, approvalRequested: true }).validation.reasonCodes).toContain("APPROVAL_DETECTED");
    expect(sealDependencyHealthAnalysis({ ...base, recommendationScoringRequested: true }).validation.reasonCodes).toContain("SCORING_DETECTED");
    expect(sealDependencyHealthAnalysis({ ...base, resourceAllocationRequested: true }).validation.reasonCodes).toContain("RESOURCE_ALLOCATION_DETECTED");
    expect(sealDependencyHealthAnalysis({ ...base, authorityExpansionDetected: true }).validation.reasonCodes).toContain("AUTHORITY_EXPANSION_DETECTED");
  }, 30000);

  it("bounds analysis references at declared limits", () => {
    const base = dependencyHealthAnalysisInput();
    const overflow = Object.freeze(Array.from(
      { length: 10_001 },
      (_, index) => `analysis:overflow:${index.toString().padStart(5, "0")}`,
    ));
    const limited = sealDependencyHealthAnalysis({
      ...base,
      foundation: {
        ...base.foundation,
        evidencePath: {
          ...base.foundation.evidencePath,
          governanceReferences: overflow,
        },
      },
    });

    expect(limited.result.analysisState).toBe("LIMITED");
    expect(limited.validation.reasonCodes).toContain("ANALYSIS_REFERENCE_LIMIT_EXCEEDED");
  }, 30000);
});
