...
import {
  ...
  buildDependencyHealthReplayRequest,
  createDependencyHealthReplayEvidencePath,
  sealDependencyHealthAnalysis,
  sealDependencyHealthObservability,
  sealDependencyHealthReplay,
  ...
  type DependencyHealthReplayInput,
...
function dependencyHealthReplayInput(
  overrides: Partial<DependencyHealthReplayInput> = {},
  ...
  const observability = overrides.observability ?? sealDependencyHealthObservability(dependencyHealthObservabilityInput({
    foundation,
    analysis,
    recommendations: overrides.recommendations ?? foundationBase.recommendations,
  }));
  ...
  return Object.freeze({
    request: overrides.request ?? buildDependencyHealthReplayRequest({
      tenantId: "tenant-alpha",
      replayScope: "FULL",
      replayVersion: "dependency-health-replay/v1",
      graphVersion: "decision-graph/v1",
    }),
    foundation,
    analysis,
    ...
    resourceAllocationRequested: overrides.resourceAllocationRequested,
    authorityExpansionDetected: overrides.authorityExpansionDetected,
  } satisfies DependencyHealthReplayInput);
}

describe("dependencyHealthReplayFramework", () => {
  it("is deterministic and produces stable replay and reconstruction hashes", () => {
    const input = dependencyHealthReplayInput();
    const first = sealDependencyHealthReplay(input);
    const second = sealDependencyHealthReplay(input);
  ...
  it("keeps reconstruction ordering deterministic when recommendations are reversed", () => {
    const base = dependencyHealthReplayInput();
    const reordered = dependencyHealthReplayInput({
      recommendations: Object.freeze([...base.recommendations].reverse()),
    });
    const sealed = sealDependencyHealthReplay(reordered);

    expect(sealed).toEqual(sealDependencyHealthReplay(reordered));
    expect(createDependencyHealthReplayEvidencePath(reordered)).toEqual(sealed.evidencePath);
  }, 90000);

  it("reconstructs health, stability, availability, continuity, recoverability, degradation, risk, governance, and observability reproducibly", () => {
    const sealed = sealDependencyHealthReplay(dependencyHealthReplayInput());

    expect(sealed.result.healthReconstructed).toBe(true);
    expect(sealed.result.stabilityReconstructed).toBe(true);
    expect(sealed.result.availabilityReconstructed).toBe(true);
    expect(sealed.result.continuityReconstructed).toBe(true);
    expect(sealed.result.recoverabilityReconstructed).toBe(true);
    expect(sealed.result.degradationReconstructed).toBe(true);
    expect(sealed.result.riskReconstructed).toBe(true);
    expect(sealed.result.governanceReconstructed).toBe(true);
  }, 90000);

  it("surfaces REPLAYABLE, LIMITED, ESCALATED, and INVALID states", () => {
    const base = dependencyHealthReplayInput();
    const replayable = sealDependencyHealthReplay(base);
    const limited = sealDependencyHealthReplay({
      ...base,
      foundation: {
        ...base.foundation,
        evidencePath: {
    ...
    const escalated = sealDependencyHealthReplay({
      ...base,
      observability: {
        ...base.observability,
        evidencePath: {
    ...
    const invalid = sealDependencyHealthReplay({
      ...base,
      authorityExpansionDetected: true,
    });

    expect(replayable.result.replayState).toBe("REPLAYABLE");
    expect(limited.result.replayState).toBe("LIMITED");
    expect(escalated.result.replayState).toBe("ESCALATED");
    expect(invalid.result.replayState).toBe("INVALID");
  }, 60000);

  it("blocks cross-tenant replay, ownership mismatches, execution, mutation, routing, prioritization, ranking, approval, scoring, resource allocation, and authority expansion", () => {
    const base = dependencyHealthReplayInput();
    const crossTenant = sealDependencyHealthReplay({
      ...base,
      dependencyRiskFoundation: {
    ...
    const ownershipMismatch = sealDependencyHealthReplay({
      ...base,
    ...
    expect(sealDependencyHealthReplay({ ...base, executionRequested: true }).validation.reasonCodes).toContain("EXECUTION_REQUEST_BLOCKED");
    expect(sealDependencyHealthReplay({ ...base, replayMutationAttempted: true }).validation.reasonCodes).toContain("REPLAY_MUTATION_DETECTED");
    expect(sealDependencyHealthReplay({ ...base, workflowRoutingRequested: true }).validation.reasonCodes).toContain("WORKFLOW_ROUTING_DETECTED");
    expect(sealDependencyHealthReplay({ ...base, prioritizationRequested: true }).validation.reasonCodes).toContain("PRIORITIZATION_DETECTED");
    expect(sealDependencyHealthReplay({ ...base, recommendationRankingRequested: true }).validation.reasonCodes).toContain("RANKING_DETECTED");
    expect(sealDependencyHealthReplay({ ...base, approvalRequested: true }).validation.reasonCodes).toContain("APPROVAL_DETECTED");
    expect(sealDependencyHealthReplay({ ...base, recommendationScoringRequested: true }).validation.reasonCodes).toContain("SCORING_DETECTED");
    expect(sealDependencyHealthReplay({ ...base, resourceAllocationRequested: true }).validation.reasonCodes).toContain("RESOURCE_ALLOCATION_DETECTED");
    expect(sealDependencyHealthReplay({ ...base, authorityExpansionDetected: true }).validation.reasonCodes).toContain("AUTHORITY_EXPANSION_DETECTED");
  }, 90000);

  it("bounds replay, lineage, and governance references at declared limits", () => {
    const base = dependencyHealthReplayInput();
