...
describe("recommendationDependencyHealthFoundation", () => {
  it("is deterministic and produces stable health graph hashes", () => {
    const input = dependencyHealthFoundationInput();
    const first = sealRecommendationDependencyHealthFoundation(input);
    const second = sealRecommendationDependencyHealthFoundation(input);

    expect(first).toEqual(second);
    expect(first.result.overallHealthState).toBe("HEALTHY");
    expect(first.result.healthGraphHash).toHaveLength(64);
  }, 30000);

  it("keeps health ordering deterministic when recommendations are reversed", () => {
    const base = dependencyHealthFoundationInput();
    const reversedRecommendations = Object.freeze([...base.recommendations].reverse());
    const first = sealRecommendationDependencyHealthFoundation(base);
    const second = sealRecommendationDependencyHealthFoundation({
      ...base,
      recommendations: reversedRecommendations,
    });

    expect(second.healthRecords).toEqual(first.healthRecords);
    expect(second.evidencePath).toEqual(first.evidencePath);
    expect(second.result.healthGraphHash).toBe(first.result.healthGraphHash);
  }, 30000);

  it("builds all seven dependency health domains reproducibly", () => {
    const sealed = sealRecommendationDependencyHealthFoundation(dependencyHealthFoundationInput());

    expect(sealed.result.healthRecordsCreated).toBeGreaterThan(0);
    expect(new Set(sealed.healthRecords.map((record) => record.healthType))).toEqual(new Set([
    ...
    expect(sealed.result.stabilityRecordsDetected).toBeGreaterThan(0);
    expect(sealed.result.availabilityRecordsDetected).toBeGreaterThan(0);
    expect(sealed.result.continuityRecordsDetected).toBeGreaterThan(0);
    expect(sealed.result.recoverabilityRecordsDetected).toBeGreaterThan(0);
    expect(sealed.result.degradationRecordsDetected).toBeGreaterThan(0);
    expect(sealed.result.riskRecordsDetected).toBeGreaterThan(0);
    expect(sealed.result.observabilityRecordsDetected).toBeGreaterThan(0);
  }, 30000);

  it("surfaces HEALTHY, STABLE, DEGRADED, AT_RISK, UNHEALTHY, and UNKNOWN states", () => {
    const base = dependencyHealthFoundationInput();
    const healthy = sealRecommendationDependencyHealthFoundation(base);
    const stable = sealRecommendationDependencyHealthFoundation({
      ...base,
      dependencyFoundation: {
        ...base.dependencyFoundation,
        result: {
          ...base.dependencyFoundation.result,
          dependencyState: "LIMITED",
        },
      },
    });
    const degraded = sealRecommendationDependencyHealthFoundation({
      ...base,
      dependencyRiskFoundation: {
        ...base.dependencyRiskFoundation,
        result: {
          ...base.dependencyRiskFoundation.result,
          dependencyRiskState: "MODERATE",
        },
      },
    });
    const atRisk = sealRecommendationDependencyHealthFoundation({
      ...base,
      dependencyRiskFoundation: {
        ...base.dependencyRiskFoundation,
        result: {
          ...base.dependencyRiskFoundation.result,
          dependencyRiskState: "HIGH",
        },
      },
    });
    const unhealthy = sealRecommendationDependencyHealthFoundation({
      ...base,
      resilienceFoundation: {
    ...
    const unknown = sealRecommendationDependencyHealthFoundation({
      ...base,
      recommendations: Object.freeze([
        {
          ...
          binding: {
            ...base.recommendations[0].binding,
            evidencePath: {
              ...base.recommendations[0].binding.evidencePath,
              governanceReferences: [],
            },
          },
          authorityScope: {
            ...base.recommendations[0].authorityScope,
            evidencePath: {
              ...base.recommendations[0].authorityScope.evidencePath,
              governanceReferences: [],
            },
          },
          policyVisibility: {
            ...base.recommendations[0].policyVisibility,
            evidencePath: {
              ...base.recommendations[0].policyVisibility.evidencePath,
              governanceReferences: [],
            },
          },
          governanceReplay: {
          ...
          governanceCertification: {
            ...
            evidencePath: {
              ...base.recommendations[0].governanceCertification.evidencePath,
              governanceReferences: [],
            },
          },
          readiness: {
            ...base.recommendations[0].readiness,
            evidencePath: {
              ...base.recommendations[0].readiness.evidencePath,
              governanceReferences: [],
            },
          },
          alignment: {
            ...base.recommendations[0].alignment,
            evidencePath: {
              ...base.recommendations[0].alignment.evidencePath,
              governanceReferences: [],
            },
          },
        },
    ...
    expect(healthy.result.overallHealthState).toBe("HEALTHY");
    expect(stable.result.overallHealthState).toBe("STABLE");
    expect(degraded.result.overallHealthState).toBe("DEGRADED");
    expect(atRisk.result.overallHealthState).toBe("AT_RISK");
    expect(unhealthy.result.overallHealthState).toBe("UNHEALTHY");
    expect(unknown.result.overallHealthState).toBe("UNKNOWN");
  }, 60000);

  it("blocks cross-tenant inputs and ownership mismatches", () => {
    const base = dependencyHealthFoundationInput();
    const crossTenant = sealRecommendationDependencyHealthFoundation({
      ...base,
      dependencyRiskFoundation: {
        ...base.dependencyRiskFoundation,
        result: {
    ...
    const ownershipMismatch = sealRecommendationDependencyHealthFoundation({
      ...base,
      recommendations: mismatchedRecommendations,
    });

    expect(crossTenant.result.overallHealthState).toBe("UNHEALTHY");
    expect(crossTenant.validation.reasonCodes).toContain("CROSS_TENANT_HEALTH_BLOCKED");
    expect(ownershipMismatch.result.overallHealthState).toBe("UNHEALTHY");
    expect(ownershipMismatch.validation.reasonCodes).toContain("OWNERSHIP_MISMATCH");
  }, 30000);

  it("fails closed on governance corruption and replay corruption", () => {
    const base = dependencyHealthFoundationInput();
    const governanceBroken = sealRecommendationDependencyHealthFoundation({
      ...base,
      constraintCertification: {
        ...base.constraintCertification,
        result: {
          ...base.constraintCertification.result,
          governanceCertified: false,
        },
      },
    });
    const replayBroken = sealRecommendationDependencyHealthFoundation({
      ...base,
      dependencyReplay: {
    ...
    expect(governanceBroken.result.overallHealthState).toBe("UNHEALTHY");
    expect(governanceBroken.validation.reasonCodes).toContain("GOVERNANCE_CORRUPTION_DETECTED");
    expect(replayBroken.result.overallHealthState).toBe("UNHEALTHY");
  ...
  it("blocks execution, mutation, routing, approval, prioritization, ranking, scoring, resource allocation, and authority expansion", () => {
    const base = dependencyHealthFoundationInput();
    expect(sealRecommendationDependencyHealthFoundation({ ...base, executionRequested: true }).validation.reasonCodes).toContain("EXECUTION_REQUEST_BLOCKED");
    expect(sealRecommendationDependencyHealthFoundation({ ...base, healthMutationAttempted: true }).validation.reasonCodes).toContain("HEALTH_MUTATION_DETECTED");
    expect(sealRecommendationDependencyHealthFoundation({ ...base, workflowRoutingRequested: true }).validation.reasonCodes).toContain("WORKFLOW_ROUTING_DETECTED");
    expect(sealRecommendationDependencyHealthFoundation({ ...base, prioritizationRequested: true }).validation.reasonCodes).toContain("PRIORITIZATION_DETECTED");
    expect(sealRecommendationDependencyHealthFoundation({ ...base, recommendationRankingRequested: true }).validation.reasonCodes).toContain("RANKING_DETECTED");
    expect(sealRecommendationDependencyHealthFoundation({ ...base, approvalRequested: true }).validation.reasonCodes).toContain("APPROVAL_DETECTED");
    expect(sealRecommendationDependencyHealthFoundation({ ...base, recommendationScoringRequested: true }).validation.reasonCodes).toContain("SCORING_DETECTED");
    expect(sealRecommendationDependencyHealthFoundation({ ...base, resourceAllocationRequested: true }).validation.reasonCodes).toContain("RESOURCE_ALLOCATION_DETECTED");
    expect(sealRecommendationDependencyHealthFoundation({ ...base, authorityExpansionDetected: true }).validation.reasonCodes).toContain("AUTHORITY_EXPANSION_DETECTED");
  }, 30000);
});
