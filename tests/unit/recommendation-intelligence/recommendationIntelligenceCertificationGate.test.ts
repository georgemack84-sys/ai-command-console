...
import {
  ...
  buildRecommendationIntelligenceCertificationRequest,
  ...
  sealRecommendationIntelligenceCertificationGate,
  ...
  type RecommendationIntelligenceCertificationInput,
...
function certificationInput(
  ...
  observabilityOverrides: Partial<RecommendationIntelligenceObservabilityInput> = {},
  replayOverrides: Partial<RecommendationIntelligenceReplayInput> = {},
  certificationOverrides: Partial<RecommendationIntelligenceCertificationInput> = {},
): RecommendationIntelligenceCertificationInput {
  const baseReplayInput = replayInput(completionOverrides, crossOverrides, observabilityOverrides, replayOverrides);
  return {
    ...baseReplayInput,
    request: buildRecommendationIntelligenceCertificationRequest({
      tenantId: "tenant-alpha",
      graphVersion: "decision-graph/v1",
    }),
    completion: sealRecommendationIntelligenceCompletionContract(completionInput(completionOverrides)),
    validationRecord: sealCrossDomainValidationEngine(validationInput(completionOverrides, crossOverrides)),
    observabilityRecord: sealRecommendationIntelligenceObservabilityLayer(observabilityInput(completionOverrides, crossOverrides, observabilityOverrides)),
    replayRecord: sealRecommendationIntelligenceReplayFramework(baseReplayInput),
    ...certificationOverrides,
  };
}

describe("recommendationIntelligenceCertificationGate", () => {
  it("is deterministic and produces stable certification hashes", () => {
    const first = sealRecommendationIntelligenceCertificationGate(certificationInput());
    const second = sealRecommendationIntelligenceCertificationGate(certificationInput());
  ...
  it("reproduces PASS, CONDITIONAL_PASS, and FAIL states", () => {
    const pass = sealRecommendationIntelligenceCertificationGate(certificationInput());
    const conditional = sealRecommendationIntelligenceCertificationGate(certificationInput({
      dependencyReplay: replay("dependency", "LIMITED") as unknown as RecommendationIntelligenceCompletionInput["dependencyReplay"],
    }));
    const fail = sealRecommendationIntelligenceCertificationGate(certificationInput(
      {},
      {
    ...
    expect(pass.result.certificationState).toBe("PASS");
    expect(conditional.result.certificationState).toBe("CONDITIONAL_PASS");
    expect(fail.result.certificationState).toBe("FAIL");
  });

  it("preserves certification booleans for completion, validation, observability, replay, governance, lineage, evidence, and tenant isolation", () => {
    const sealed = sealRecommendationIntelligenceCertificationGate(certificationInput());

    expect(sealed.result.completionCertified).toBe(true);
    expect(sealed.result.validationCertified).toBe(true);
    ...
    expect(sealed.result.lineageCertified).toBe(true);
    expect(sealed.result.evidenceCertified).toBe(true);
    expect(sealed.result.tenantIsolationCertified).toBe(true);
  });

  it("blocks cross-tenant certification and ownership mismatches", () => {
    const crossTenant = sealRecommendationIntelligenceCertificationGate(certificationInput({
    ...
    const ownershipMismatch = sealRecommendationIntelligenceCertificationGate(certificationInput({
  ...
  it("preserves deterministic evidence ordering regardless of recommendation ordering", () => {
    ...
    expect(sealRecommendationIntelligenceCertificationGate(reversed).evidencePath).toEqual(
      sealRecommendationIntelligenceCertificationGate(certificationInput()).evidencePath,
    );
  });
  ...
  it("enforces non-control boundaries", () => {
    ...
    expect(sealRecommendationIntelligenceCertificationGate({ ...base, executionRequested: true }).validation.reasonCodes).toContain("EXECUTION_REQUEST_BLOCKED");
    expect(sealRecommendationIntelligenceCertificationGate({ ...base, workflowRoutingRequested: true }).validation.reasonCodes).toContain("WORKFLOW_ROUTING_DETECTED");
    expect(sealRecommendationIntelligenceCertificationGate({ ...base, approvalRequested: true }).validation.reasonCodes).toContain("APPROVAL_DETECTED");
    expect(sealRecommendationIntelligenceCertificationGate({ ...base, recommendationRankingRequested: true }).validation.reasonCodes).toContain("RANKING_DETECTED");
    expect(sealRecommendationIntelligenceCertificationGate({ ...base, prioritizationRequested: true }).validation.reasonCodes).toContain("PRIORITIZATION_DETECTED");
    expect(sealRecommendationIntelligenceCertificationGate({ ...base, recommendationScoringRequested: true }).validation.reasonCodes).toContain("SCORING_DETECTED");
    expect(sealRecommendationIntelligenceCertificationGate({ ...base, resourceAllocationRequested: true }).validation.reasonCodes).toContain("RESOURCE_ALLOCATION_DETECTED");
    expect(sealRecommendationIntelligenceCertificationGate({ ...base, certificationMutationAttempted: true }).validation.reasonCodes).toContain("CERTIFICATION_MUTATION_DETECTED");
  });
});
