...
import {
  ...
  createGovernanceBindingCertificationEvidencePath,
...
function graphInput(overrides: Partial<DecisionGraphContractInput> = {}): DecisionGraphContractInput {
  const nodes: DecisionGraphNodeInput[] = [
    { nodeId: "boundary-node", graphId: "graph-59e", nodeType: "CONSTRAINT", tenantId: "tenant-alpha", lineageReference: "lineage-boundary-node" },
    { nodeId: "governance-anchor", graphId: "graph-59e", nodeType: "GOVERNANCE", tenantId: "tenant-alpha", lineageReference: "lineage-governance-anchor" },
    { nodeId: "recommendation-anchor", graphId: "graph-59e", nodeType: "RECOMMENDATION", tenantId: "tenant-alpha", lineageReference: "lineage-recommendation-anchor" },
    { nodeId: "escalation-anchor", graphId: "graph-59e", nodeType: "ESCALATION", tenantId: "tenant-alpha", lineageReference: "lineage-escalation-anchor" },
  ];
...
const proposalNodes = (): readonly ProposalNodeInput[] => [
  { proposalId: "proposal-a", graphId: "graph-59e", tenantId: "tenant-alpha", proposalType: "MISSION", lineageReference: "lineage-proposal-a" },
];
const governanceNodes = (): readonly GovernanceNodeInput[] => [
  { governanceId: "governance-a", graphId: "graph-59e", tenantId: "tenant-alpha", governanceType: "POLICY", lineageReference: "lineage-governance-a" },
];
const escalationNodes = (): readonly EscalationNodeInput[] => [
  { escalationId: "escalation-a", graphId: "graph-59e", tenantId: "tenant-alpha", escalationType: "REVIEW", lineageReference: "lineage-escalation-a" },
];
...
function certificationInput(overrides: Partial<GovernanceBindingCertificationInput> = {}): GovernanceBindingCertificationInput {
  const base = buildCertificationInputs();
  const request = buildGovernanceBindingCertificationRequest({
    ...base,
    recommendationId: "recommendation-anchor",
    tenantId: "tenant-alpha",
    certificationScope: "FULL",
...
describe("governanceBindingCertificationGate", () => {
  it("is deterministic and reproduces certification hashes", () => {
    const input = certificationInput();
    const first = sealGovernanceBindingCertification(input);
    const second = sealGovernanceBindingCertification(input);
  ...
  it("keeps evidence ordering reproducible across scopes", () => {
    ...
    const scopes = ["BINDINGS", "AUTHORITY", "POLICY", "REPLAY", "FULL"] as const;
    for (const certificationScope of scopes) {
      const scoped = certificationInput({ request: { ...base.request, certificationScope } });
      expect(createGovernanceBindingCertificationEvidencePath(scoped)).toEqual(
        createGovernanceBindingCertificationEvidencePath(scoped),
      );
      expect(sealGovernanceBindingCertification(scoped).result.certificationHash).toBe(
        sealGovernanceBindingCertification(scoped).result.certificationHash,
      );
    }
  });
  ...
  it("blocks cross-tenant certification and ownership mismatch", () => {
    const base = certificationInput();
    const crossTenant = sealGovernanceBindingCertification({
      ...base,
      replayEvidence: { ...base.replayEvidence, tenantId: "tenant-beta" },
    });
    const ownershipMismatch = sealGovernanceBindingCertification({
      ...base,
  ...
  it("produces CONDITIONAL_PASS for bounded replay degradation", () => {
    const base = certificationInput();
    const degraded = sealGovernanceBindingCertification({
      ...base,
    ...
    expect(degraded.validation.reasonCodes).toContain("REPLAY_DEGRADED");
  });

  it("fails on broken bindings, policy visibility, and authority concerns", () => {
    const base = certificationInput();
    const brokenBindings = sealGovernanceBindingCertification({
      ...base,
      binding: { ...base.binding, result: { ...base.binding.result, governanceBound: false, bindingState: "INVALID" } },
    });
    const brokenPolicy = sealGovernanceBindingCertification({
      ...base,
      policyVisibility: { ...base.policyVisibility, result: { ...base.policyVisibility.result, policiesVisible: false, visibilityState: "INVALID" } },
    });
    const brokenAuthority = sealGovernanceBindingCertification({
      ...base,
      authorityScope: { ...base.authorityScope, result: { ...base.authorityScope.result, scopeValidated: false, scopeState: "INVALID" } },
    });
    expect(brokenBindings.result.certificationState).toBe("FAIL");
    expect(brokenBindings.validation.reasonCodes).toContain("GOVERNANCE_BINDING_BROKEN");
    expect(brokenPolicy.validation.reasonCodes).toContain("POLICY_VISIBILITY_BROKEN");
    expect(brokenAuthority.validation.reasonCodes).toContain("AUTHORITY_CONCERN_DETECTED");
  });

  it("fails on certification mutation, hidden governance state, and authority expansion", () => {
    const base = certificationInput();
    expect(sealGovernanceBindingCertification({ ...base, certificationMutationAttempted: true }).validation.reasonCodes).toContain("CERTIFICATION_MUTATION_DETECTED");
    expect(sealGovernanceBindingCertification({ ...base, hiddenGovernanceStateDetected: true }).validation.reasonCodes).toContain("HIDDEN_GOVERNANCE_STATE_DETECTED");
    expect(sealGovernanceBindingCertification({ ...base, authorityExpansionDetected: true }).validation.reasonCodes).toContain("AUTHORITY_EXPANSION_DETECTED");
  });

  it("blocks execution, workflow, approval, policy execution, and prioritization behavior", () => {
    const base = certificationInput();
    expect(sealGovernanceBindingCertification({ ...base, executionRequested: true }).validation.reasonCodes).toContain("EXECUTION_REQUEST_BLOCKED");
    expect(sealGovernanceBindingCertification({ ...base, workflowRoutingRequested: true }).validation.reasonCodes).toContain("WORKFLOW_ROUTING_DETECTED");
    expect(sealGovernanceBindingCertification({ ...base, approvalBehaviorRequested: true }).validation.reasonCodes).toContain("APPROVAL_BEHAVIOR_DETECTED");
    expect(sealGovernanceBindingCertification({ ...base, policyExecutionRequested: true }).validation.reasonCodes).toContain("POLICY_EXECUTION_DETECTED");
    expect(sealGovernanceBindingCertification({ ...base, recommendationPrioritizationRequested: true }).validation.reasonCodes).toContain("RECOMMENDATION_PRIORITIZATION_DETECTED");
  });

  it("does not mutate inputs", () => {
    const input = certificationInput();
    const before = JSON.stringify(input);
    sealGovernanceBindingCertification(input);
