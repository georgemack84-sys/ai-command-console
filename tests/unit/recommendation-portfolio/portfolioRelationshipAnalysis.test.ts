...
import {
  ...
  createPortfolioRelationshipAnalysisEvidencePath,
...
function portfolioInput(overrides: Partial<RecommendationPortfolioInput> = {}): RecommendationPortfolioInput {
  const alpha = buildBundle("recommendation-alpha", "graph-511b-alpha");
  const beta = buildBundle("recommendation-beta", "graph-511b-beta");
...
function relationshipInput(overrides: Partial<PortfolioRelationshipAnalysisInput> = {}): PortfolioRelationshipAnalysisInput {
  const basePortfolioInput = portfolioInput();
  const portfolio = sealRecommendationPortfolio(basePortfolioInput);
  ...
  return Object.freeze({
    request,
    portfolio,
    recommendations: basePortfolioInput.recommendations,
    ...overrides,
  } satisfies PortfolioRelationshipAnalysisInput);
}

describe("portfolioRelationshipAnalysis", () => {
  it("is deterministic and reproduces analysis hashes", () => {
    const input = relationshipInput();
    const first = sealPortfolioRelationshipAnalysis(input);
    const second = sealPortfolioRelationshipAnalysis(input);
    expect(first).toEqual(second);
    expect(first.result.relationshipState).toBe("ANALYZED");
    expect(first.result.analysisHash).toHaveLength(64);
  });

  it("detects shared relationships reproducibly and orders them deterministically", () => {
    ...
    const recommendations = Object.freeze([
      base.recommendations[0],
      {
        ...
        governanceReferences: {
          ...base.recommendations[1].governanceReferences,
          governanceReferences: Object.freeze([...base.recommendations[0].governanceReferences.governanceReferences]),
        },
        replayEvidence: {
          ...base.recommendations[1].replayEvidence,
          replayReferences: Object.freeze([...base.recommendations[0].replayEvidence.replayReferences]),
        },
      },
    ]);
    const sealed = sealPortfolioRelationshipAnalysis({ ...base, recommendations });
    const reversed = sealPortfolioRelationshipAnalysis({ ...base, recommendations: Object.freeze([...recommendations].reverse()) });
    expect(sealed.relationships.length).toBeGreaterThan(0);
    expect(reversed.relationships).toEqual(sealed.relationships);
    expect(createPortfolioRelationshipAnalysisEvidencePath({ ...base, recommendations }, sealed.relationships)).toEqual(
      createPortfolioRelationshipAnalysisEvidencePath({ ...base, recommendations }, sealed.relationships),
    );
  });

  it("preserves governance, replay, and lineage relationship counts", () => {
    const base = relationshipInput();
    const recommendations = Object.freeze([
      {
        ...base.recommendations[0],
        governanceReferences: {
          ...
          governanceReferences: Object.freeze(["gov:shared", "gov:oversight"]),
        },
      },
      {
        ...base.recommendations[1],
        ...
        replayEvidence: {
          ...base.recommendations[1].replayEvidence,
          replayReferences: Object.freeze(["replay:shared"]),
        },
      },
    ]);
    const replayAligned = Object.freeze([
      {
        ...recommendations[0],
        replayEvidence: {
          ...recommendations[0].replayEvidence,
          replayReferences: Object.freeze(["replay:shared"]),
        },
      },
      recommendations[1],
    ]);
    const sealed = sealPortfolioRelationshipAnalysis({ ...base, recommendations: replayAligned });
    expect(sealed.result.governanceRelationshipsDetected).toBeGreaterThan(0);
    expect(sealed.result.replayRelationshipsDetected).toBeGreaterThan(0);
    expect(sealed.result.lineageRelationshipsDetected).toBeGreaterThanOrEqual(0);
  });

  it("surfaces replay degradation as LIMITED", () => {
    const base = relationshipInput();
    const limited = sealPortfolioRelationshipAnalysis({
      ...base,
    ...
    expect(limited.result.relationshipState).toBe("LIMITED");
    expect(limited.validation.reasonCodes).toContain("REPLAY_DEGRADED");
  });

  it("surfaces missing relationship evidence as OBSERVE", () => {
    const base = relationshipInput();
    const observe = sealPortfolioRelationshipAnalysis({
      ...base,
      portfolio: {
        ...base.portfolio,
        portfolio: {
          ...base.portfolio.portfolio,
          recommendationIds: [...base.portfolio.portfolio.recommendationIds, "recommendation-gamma"],
        },
      },
    });
    expect(observe.result.relationshipState).toBe("OBSERVE");
    expect(observe.validation.reasonCodes).toContain("RELATIONSHIP_EVIDENCE_MISSING");
  });

  it("blocks cross-tenant relationships and ownership mismatch", () => {
    const base = relationshipInput();
    const crossTenant = sealPortfolioRelationshipAnalysis({
      ...base,
      recommendations: Object.freeze([
    ...
    const ownershipMismatch = sealPortfolioRelationshipAnalysis({
      ...base,
      recommendations: Object.freeze([
    ...
    expect(crossTenant.result.relationshipState).toBe("INVALID");
    expect(crossTenant.validation.reasonCodes).toContain("CROSS_TENANT_RELATIONSHIPS_BLOCKED");
  ...
  it("fails on governance corruption and authority expansion", () => {
    const base = relationshipInput();
    const brokenGovernance = sealPortfolioRelationshipAnalysis({
      ...base,
      recommendations: Object.freeze([
    ...
    const authorityExpanded = sealPortfolioRelationshipAnalysis({ ...base, authorityExpansionDetected: true });
    expect(brokenGovernance.result.relationshipState).toBe("INVALID");
  ...
  it("blocks execution, mutation, ranking, prioritization, scoring, approval, and workflow routing", () => {
    const base = relationshipInput();
    expect(sealPortfolioRelationshipAnalysis({ ...base, executionRequested: true }).validation.reasonCodes).toContain("EXECUTION_REQUEST_BLOCKED");
    expect(sealPortfolioRelationshipAnalysis({ ...base, analysisMutationAttempted: true }).validation.reasonCodes).toContain("ANALYSIS_MUTATION_DETECTED");
    expect(sealPortfolioRelationshipAnalysis({ ...base, recommendationRankingRequested: true }).validation.reasonCodes).toContain("RECOMMENDATION_RANKING_DETECTED");
    expect(sealPortfolioRelationshipAnalysis({ ...base, recommendationPrioritizationRequested: true }).validation.reasonCodes).toContain("RECOMMENDATION_PRIORITIZATION_DETECTED");
    expect(sealPortfolioRelationshipAnalysis({ ...base, recommendationScoringRequested: true }).validation.reasonCodes).toContain("RECOMMENDATION_SCORING_DETECTED");
    expect(sealPortfolioRelationshipAnalysis({ ...base, recommendationApprovalRequested: true }).validation.reasonCodes).toContain("RECOMMENDATION_APPROVAL_DETECTED");
    expect(sealPortfolioRelationshipAnalysis({ ...base, workflowRoutingRequested: true }).validation.reasonCodes).toContain("WORKFLOW_ROUTING_DETECTED");
  });

  it("does not mutate inputs", () => {
    const input = relationshipInput();
    const before = JSON.stringify(input);
    sealPortfolioRelationshipAnalysis(input);
