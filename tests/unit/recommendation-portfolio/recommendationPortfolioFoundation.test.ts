...
import {
  ...
  createRecommendationPortfolioEvidencePath,
...
function portfolioInput(
  overrides: Partial<RecommendationPortfolioInput> = {},
): RecommendationPortfolioInput {
  const alpha = buildBundle("recommendation-alpha", "graph-511a-alpha");
  const beta = buildBundle("recommendation-beta", "graph-511a-beta");
...
describe("recommendationPortfolioFoundation", () => {
  it("is deterministic and reproduces portfolio hashes", () => {
    const input = portfolioInput();
    const first = sealRecommendationPortfolio(input);
    const second = sealRecommendationPortfolio(input);
    expect(first).toEqual(second);
    expect(first.result.portfolioState).toBe("ESTABLISHED");
    expect(first.result.portfolioHash).toHaveLength(64);
  });

  it("keeps portfolio ordering and duplicate handling deterministic", () => {
    const base = portfolioInput();
    const reordered = portfolioInput({
      request: { ...base.request, recommendationIds: ["recommendation-alpha", "recommendation-beta", "recommendation-alpha"] },
      recommendations: Object.freeze([...base.recommendations].reverse()),
    });
    const sealed = sealRecommendationPortfolio(reordered);
    expect(sealed.portfolio.recommendationIds).toEqual(["recommendation-alpha", "recommendation-beta"]);
    expect(createRecommendationPortfolioEvidencePath(reordered)).toEqual(createRecommendationPortfolioEvidencePath(reordered));
  });

  it("preserves governance, replay, and lineage references", () => {
    const sealed = sealRecommendationPortfolio(portfolioInput());
    expect(sealed.evidencePath.governanceReferences.length).toBeGreaterThan(0);
    ...
    expect(sealed.validation.governanceBound).toBe(true);
    expect(sealed.validation.lineageBound).toBe(true);
  });

  it("surfaces replay degradation as LIMITED", () => {
    const base = portfolioInput();
    const degraded = sealRecommendationPortfolio({
      ...base,
    ...
    expect(degraded.result.portfolioState).toBe("LIMITED");
    expect(degraded.validation.reasonCodes).toContain("REPLAY_DEGRADED");
  });

  it("surfaces incomplete evidence as OBSERVE", () => {
    const base = portfolioInput();
    const observe = sealRecommendationPortfolio({
      ...base,
      request: { ...base.request, recommendationIds: ["recommendation-alpha", "recommendation-beta", "recommendation-gamma"] },
    });
    expect(observe.result.portfolioState).toBe("OBSERVE");
    expect(observe.validation.reasonCodes).toContain("PORTFOLIO_INCOMPLETE");
  });

  it("blocks cross-tenant portfolios and ownership mismatch", () => {
    const base = portfolioInput();
    const crossTenant = sealRecommendationPortfolio({
      ...base,
      recommendations: Object.freeze([
    ...
    const ownershipMismatch = sealRecommendationPortfolio({
      ...base,
      recommendations: Object.freeze([
    ...
    expect(crossTenant.result.portfolioState).toBe("INVALID");
    expect(crossTenant.validation.reasonCodes).toContain("CROSS_TENANT_RECOMMENDATIONS_BLOCKED");
  ...
  it("fails on governance corruption and authority expansion", () => {
    const base = portfolioInput();
    const brokenGovernance = sealRecommendationPortfolio({
      ...base,
      recommendations: Object.freeze([
    ...
    const authorityExpanded = sealRecommendationPortfolio({ ...base, authorityExpansionDetected: true });
    expect(brokenGovernance.result.portfolioState).toBe("INVALID");
  ...
  it("blocks execution, mutation, ranking, prioritization, approval, and workflow routing", () => {
    const base = portfolioInput();
    expect(sealRecommendationPortfolio({ ...base, executionRequested: true }).validation.reasonCodes).toContain("EXECUTION_REQUEST_BLOCKED");
    expect(sealRecommendationPortfolio({ ...base, portfolioMutationAttempted: true }).validation.reasonCodes).toContain("PORTFOLIO_MUTATION_DETECTED");
    expect(sealRecommendationPortfolio({ ...base, recommendationRankingRequested: true }).validation.reasonCodes).toContain("RECOMMENDATION_RANKING_DETECTED");
    expect(sealRecommendationPortfolio({ ...base, recommendationPrioritizationRequested: true }).validation.reasonCodes).toContain("RECOMMENDATION_PRIORITIZATION_DETECTED");
    expect(sealRecommendationPortfolio({ ...base, recommendationApprovalRequested: true }).validation.reasonCodes).toContain("RECOMMENDATION_APPROVAL_DETECTED");
    expect(sealRecommendationPortfolio({ ...base, workflowRoutingRequested: true }).validation.reasonCodes).toContain("WORKFLOW_ROUTING_DETECTED");
  });

  it("does not mutate inputs", () => {
    const input = portfolioInput();
    const before = JSON.stringify(input);
    sealRecommendationPortfolio(input);
