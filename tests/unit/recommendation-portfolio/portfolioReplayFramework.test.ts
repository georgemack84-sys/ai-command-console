...
import {
  ...
  createPortfolioReplayEvidencePath,
...
function portfolioInput(overrides: Partial<RecommendationPortfolioInput> = {}): RecommendationPortfolioInput {
  const alpha = buildBundle("recommendation-alpha", "graph-511d-alpha");
  const beta = buildBundle("recommendation-beta", "graph-511d-beta");
...
function replayInput(overrides: Partial<PortfolioReplayInput> = {}): PortfolioReplayInput {
...
describe("portfolioReplayFramework", () => {
  it("is deterministic and reproduces replay hashes", () => {
    const input = replayInput();
    const first = sealPortfolioReplay(input);
    const second = sealPortfolioReplay(input);
  ...
  it("keeps reconstruction ordering reproducible", () => {
    const input = replayInput();
    expect(createPortfolioReplayEvidencePath(input)).toEqual(createPortfolioReplayEvidencePath(input));
    const reversed = replayInput({
      recommendations: Object.freeze([...input.recommendations].reverse()),
    });
    expect(sealPortfolioReplay(reversed)).toEqual(sealPortfolioReplay(input));
  });

  it("reconstructs composition, relationships, evidence, and governance reproducibly", () => {
    const sealed = sealPortfolioReplay(replayInput());
    expect(sealed.result.compositionReconstructed).toBe(true);
    expect(sealed.result.relationshipsReconstructed).toBe(true);
    expect(sealed.result.evidenceReconstructed).toBe(true);
    expect(sealed.result.governanceReconstructed).toBe(true);
  });

  it("surfaces missing replay artifacts or composition evidence as LIMITED", () => {
    const base = replayInput();
    const limitedReplay = sealPortfolioReplay({
      ...base,
    ...
    const limitedComposition = sealPortfolioReplay({
      ...base,
      portfolio: {
        ...base.portfolio,
        portfolio: {
    ...
    expect(limitedComposition.validation.reasonCodes).toContain("COMPOSITION_EVIDENCE_MISSING");
  });

  it("surfaces replay mismatch and lineage continuity break as ESCALATED", () => {
    const base = replayInput();
    const mismatch = sealPortfolioReplay({
      ...base,
      recommendations: Object.freeze([
        { ...base.recommendations[0], replay: { ...base.recommendations[0].replay, result: { ...base.recommendations[0].replay.result, replayState: "INVALID" } } },
        base.recommendations[1],
      ]),
    });
    const lineageBroken = sealPortfolioReplay({
      ...base,
      recommendations: Object.freeze([
  ...
  it("blocks cross-tenant replay and ownership mismatch", () => {
    const base = replayInput();
    const crossTenant = sealPortfolioReplay({
      ...base,
      recommendations: Object.freeze([
    ...
    const ownershipMismatch = sealPortfolioReplay({
      ...base,
      recommendations: Object.freeze([
  ...
  it("fails invalid on governance corruption and authority expansion", () => {
    const base = replayInput();
    const brokenGovernance = sealPortfolioReplay({
      ...base,
      recommendations: Object.freeze([
    ...
    const authorityExpanded = sealPortfolioReplay({ ...base, authorityExpansionDetected: true });
  ...
  it("blocks execution, mutation, ranking, prioritization, scoring, approval, and workflow routing", () => {
    const base = replayInput();
    expect(sealPortfolioReplay({ ...base, executionRequested: true }).validation.reasonCodes).toContain("EXECUTION_REQUEST_BLOCKED");
    expect(sealPortfolioReplay({ ...base, replayMutationAttempted: true }).validation.reasonCodes).toContain("REPLAY_MUTATION_DETECTED");
    expect(sealPortfolioReplay({ ...base, recommendationRankingRequested: true }).validation.reasonCodes).toContain("RECOMMENDATION_RANKING_DETECTED");
    expect(sealPortfolioReplay({ ...base, recommendationPrioritizationRequested: true }).validation.reasonCodes).toContain("RECOMMENDATION_PRIORITIZATION_DETECTED");
    expect(sealPortfolioReplay({ ...base, recommendationScoringRequested: true }).validation.reasonCodes).toContain("RECOMMENDATION_SCORING_DETECTED");
    expect(sealPortfolioReplay({ ...base, recommendationApprovalRequested: true }).validation.reasonCodes).toContain("RECOMMENDATION_APPROVAL_DETECTED");
    expect(sealPortfolioReplay({ ...base, workflowRoutingRequested: true }).validation.reasonCodes).toContain("WORKFLOW_ROUTING_DETECTED");
  });

  it("does not mutate inputs", () => {
    const input = replayInput();
    const before = JSON.stringify(input);
    sealPortfolioReplay(input);
