...
import {
  ...
  createDependencyObservabilityEvidencePath,
...
function portfolioInput(overrides: Partial<RecommendationPortfolioInput> = {}): RecommendationPortfolioInput {
  const alpha = buildBundle("recommendation-alpha", "graph-512c-alpha");
  const beta = buildBundle("recommendation-beta", "graph-512c-beta");
  return Object.freeze({
...
function dependencyObservabilityInput(overrides: Partial<DependencyObservabilityInput> = {}): DependencyObservabilityInput {
...
describe("dependencyObservabilityLayer", () => {
  it("is deterministic and reproduces observability hashes", () => {
    const input = dependencyObservabilityInput();
    const first = sealDependencyObservability(input);
    const second = sealDependencyObservability(input);
  ...
  it("keeps visibility ordering reproducible", () => {
    const input = dependencyObservabilityInput();
    const reversed = dependencyObservabilityInput({ recommendations: Object.freeze([...input.recommendations].reverse()) });
    expect(sealDependencyObservability(reversed)).toEqual(sealDependencyObservability(input));
    const sealed = sealDependencyObservability(input);
    expect(createDependencyObservabilityEvidencePath(input)).toEqual(createDependencyObservabilityEvidencePath(input));
    expect(sealed.evidencePath.chainReferences).toEqual([...sealed.evidencePath.chainReferences].sort());
  });

  it("preserves graph, chain, lineage, replay, conflict, and audit visibility", () => {
    const sealed = sealDependencyObservability(dependencyObservabilityInput());
    expect(sealed.result.dependencyGraphVisible).toBe(true);
    expect(sealed.result.dependencyChainsVisible).toBe(true);
    expect(sealed.result.dependencyLineageVisible).toBe(true);
    expect(sealed.result.dependencyReplayVisible).toBe(true);
    expect(sealed.result.dependencyConflictsVisible).toBe(true);
    expect(sealed.result.dependencyAuditVisible).toBe(true);
  });

  it("surfaces replay visibility degradation as LIMITED", () => {
    const base = dependencyObservabilityInput();
    const limited = sealDependencyObservability({
      ...base,
    ...
    expect(limited.validation.reasonCodes).toContain("DEPENDENCY_REPLAY_VISIBILITY_MISSING");
  });

  it("surfaces missing visibility evidence as OBSERVE", () => {
    const base = dependencyObservabilityInput();
    const observe = sealDependencyObservability({
      ...base,
      foundation: {
        ...base.foundation,
  ...
  it("blocks cross-tenant visibility and ownership mismatch", () => {
    const base = dependencyObservabilityInput();
    const crossTenant = sealDependencyObservability({
      ...base,
      recommendations: Object.freeze([
    ...
    const ownershipMismatch = sealDependencyObservability({
      ...base,
      recommendations: Object.freeze([
  ...
  it("fails on governance corruption", () => {
    const base = dependencyObservabilityInput();
    const brokenGovernance = sealDependencyObservability({
      ...base,
      recommendations: Object.freeze([
  ...
  it("blocks execution, mutation, workflow routing, ranking, prioritization, approval, and authority expansion", () => {
    const base = dependencyObservabilityInput();
    expect(sealDependencyObservability({ ...base, executionRequested: true }).validation.reasonCodes).toContain("EXECUTION_REQUEST_BLOCKED");
    expect(sealDependencyObservability({ ...base, observabilityMutationAttempted: true }).validation.reasonCodes).toContain("OBSERVABILITY_MUTATION_DETECTED");
    expect(sealDependencyObservability({ ...base, workflowRoutingRequested: true }).validation.reasonCodes).toContain("WORKFLOW_ROUTING_DETECTED");
    expect(sealDependencyObservability({ ...base, recommendationRankingRequested: true }).validation.reasonCodes).toContain("RECOMMENDATION_RANKING_DETECTED");
    expect(sealDependencyObservability({ ...base, recommendationPrioritizationRequested: true }).validation.reasonCodes).toContain("RECOMMENDATION_PRIORITIZATION_DETECTED");
    expect(sealDependencyObservability({ ...base, recommendationApprovalRequested: true }).validation.reasonCodes).toContain("RECOMMENDATION_APPROVAL_DETECTED");
    expect(sealDependencyObservability({ ...base, authorityExpansionDetected: true }).validation.reasonCodes).toContain("AUTHORITY_EXPANSION_DETECTED");
  });
});
