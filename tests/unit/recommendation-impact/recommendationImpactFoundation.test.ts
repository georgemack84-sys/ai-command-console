...
import {
  ...
  createRecommendationImpactEvidencePath,
...
function portfolioInput(overrides: Partial<RecommendationPortfolioInput> = {}): RecommendationPortfolioInput {
  const alpha = buildBundle("recommendation-alpha", "graph-513a-alpha");
  const beta = buildBundle("recommendation-beta", "graph-513a-beta");
...
function impactInput(overrides: Partial<RecommendationImpactFoundationInput> = {}): RecommendationImpactFoundationInput {
...
describe("recommendationImpactFoundation", () => {
  it("is deterministic and reproduces impact graph hashes", () => {
    const input = impactInput();
    const first = sealRecommendationImpactFoundation(input);
    const second = sealRecommendationImpactFoundation(input);
    expect(first).toEqual(second);
    expect(first.result.impactGraphHash).toHaveLength(64);
  });

  it("keeps impact graph ordering deterministic", () => {
    const input = impactInput();
    const reversed = impactInput({ recommendations: Object.freeze([...input.recommendations].reverse()) });
    expect(sealRecommendationImpactFoundation(reversed)).toEqual(sealRecommendationImpactFoundation(input));
    expect(createRecommendationImpactEvidencePath(input, sealRecommendationImpactFoundation(input).impacts)).toEqual(
      createRecommendationImpactEvidencePath(input, sealRecommendationImpactFoundation(input).impacts),
    );
  });

  it("creates reproducible evidence, governance, replay, readiness, and portfolio impacts", () => {
    const base = impactInput();
    const aligned = sealRecommendationImpactFoundation({
      ...base,
    ...
    expect(aligned.impacts.length).toBeGreaterThan(0);
    expect(aligned.result.evidenceImpactsDetected).toBeGreaterThan(0);
    expect(aligned.result.governanceImpactsDetected).toBeGreaterThan(0);
    expect(aligned.result.replayImpactsDetected).toBeGreaterThan(0);
    expect(aligned.result.readinessImpactsDetected).toBeGreaterThan(0);
    expect(aligned.result.portfolioImpactsDetected).toBeGreaterThan(0);
  });

  it("surfaces missing impact references as LIMITED or OBSERVE", () => {
    const base = impactInput();
    const limited = sealRecommendationImpactFoundation({
      ...base,
      request: { ...base.request, recommendationIds: ["recommendation-alpha"], impactScope: "EVIDENCE" },
      recommendations: Object.freeze([base.recommendations[1]]),
    });
    const observe = sealRecommendationImpactFoundation({
      ...base,
      request: { ...base.request, recommendationIds: ["recommendation-alpha", "recommendation-beta", "recommendation-gamma"], impactScope: "PORTFOLIO" },
    });
    expect(limited.result.impactState).toBe("LIMITED");
    expect(observe.result.impactState).toBe("OBSERVE");
  });

  it("blocks cross-tenant impacts and ownership mismatch", () => {
    const base = impactInput();
    const crossTenant = sealRecommendationImpactFoundation({
      ...base,
      recommendations: Object.freeze([
    ...
    const ownershipMismatch = sealRecommendationImpactFoundation({
      ...base,
      recommendations: Object.freeze([
    ...
    expect(crossTenant.result.impactState).toBe("INVALID");
    expect(crossTenant.validation.reasonCodes).toContain("CROSS_TENANT_IMPACTS_BLOCKED");
    expect(ownershipMismatch.validation.reasonCodes).toContain("OWNERSHIP_MISMATCH");
  });

  it("fails on governance corruption, replay corruption, readiness degradation, or portfolio corruption", () => {
    const base = impactInput();
    const brokenGovernance = sealRecommendationImpactFoundation({
      ...base,
      recommendations: Object.freeze([
    ...
    const brokenReplay = sealRecommendationImpactFoundation({
      ...base,
      replay: { ...base.replay, result: { ...base.replay.result, replayState: "INVALID" } },
    });
    const brokenReadiness = sealRecommendationImpactFoundation({
      ...base,
      recommendations: Object.freeze([
        { ...base.recommendations[0], readiness: { ...base.recommendations[0].readiness, result: { ...base.recommendations[0].readiness.result, readinessState: "NOT_READY" } } },
        base.recommendations[1],
      ]),
    });
    const brokenPortfolio = sealRecommendationImpactFoundation({
      ...base,
      portfolioCertification: { ...base.portfolioCertification, result: { ...base.portfolioCertification.result, certificationState: "FAIL" } },
    });
    ...
    expect(brokenReadiness.validation.reasonCodes).toContain("READINESS_DEGRADED");
    expect(brokenPortfolio.validation.reasonCodes).toContain("PORTFOLIO_CORRUPTION_DETECTED");
  });

  it("blocks execution, mutation, workflow ordering, approval, prioritization, and authority expansion", () => {
    const base = impactInput();
    expect(sealRecommendationImpactFoundation({ ...base, executionRequested: true }).validation.reasonCodes).toContain("EXECUTION_REQUEST_BLOCKED");
    expect(sealRecommendationImpactFoundation({ ...base, impactMutationAttempted: true }).validation.reasonCodes).toContain("IMPACT_MUTATION_DETECTED");
    expect(sealRecommendationImpactFoundation({ ...base, workflowRoutingRequested: true }).validation.reasonCodes).toContain("WORKFLOW_ROUTING_DETECTED");
    expect(sealRecommendationImpactFoundation({ ...base, approvalRequested: true }).validation.reasonCodes).toContain("APPROVAL_DETECTED");
    expect(sealRecommendationImpactFoundation({ ...base, prioritizationRequested: true }).validation.reasonCodes).toContain("PRIORITIZATION_DETECTED");
    expect(sealRecommendationImpactFoundation({ ...base, authorityExpansionDetected: true }).validation.reasonCodes).toContain("AUTHORITY_EXPANSION_DETECTED");
  });

  it("does not mutate inputs", () => {
    const input = impactInput();
    const before = JSON.stringify(input);
    sealRecommendationImpactFoundation(input);
