...
describe("impactCertificationGate", () => {
  it("is deterministic and certifies integrity, propagation, replay, governance, and observability", () => {
    const input = impactCertificationInput();
    const first = sealImpactCertification(input);
    const second = sealImpactCertification(input);
    ...
    expect(first.result.replayCertified).toBe(true);
  ...
  it("keeps certification ordering reproducible", () => {
    const input = impactCertificationInput();
    const reversed = impactCertificationInput({ recommendations: Object.freeze([...input.recommendations].reverse()) });
    expect(sealImpactCertification(reversed)).toEqual(sealImpactCertification(input));
  });

  it("drops to conditional pass on replay degradation and incomplete observability", () => {
    const replayDegraded = sealImpactCertification({
      ...impactCertificationInput(),
      replay: {
        ...impactCertificationInput().replay,
        result: {
          ...impactCertificationInput().replay.result,
          replayState: "LIMITED",
        },
      },
    });
    const observabilityIncomplete = sealImpactCertification({
      ...impactCertificationInput(),
      observability: {
        ...impactCertificationInput().observability,
        result: {
          ...impactCertificationInput().observability.result,
          observabilityState: "LIMITED",
          impactAuditVisible: false,
        },
      },
    });
  ...
  it("fails on integrity, propagation, lineage, replay, and governance corruption", () => {
    const base = impactCertificationInput();
    const integrityBroken = sealImpactCertification({
      ...base,
      foundation: {
        ...base.foundation,
    ...
    const propagationBroken = sealImpactCertification({
      ...base,
      replay: {
        ...base.replay,
        result: {
          ...base.replay.result,
          propagationReconstructed: false,
        },
      },
    });
    const lineageBroken = sealImpactCertification({
      ...base,
      recommendations: Object.freeze([
    ...
    const replayCorrupted = sealImpactCertification({
      ...base,
      replay: {
        ...base.replay,
        result: {
    ...
    const governanceBroken = sealImpactCertification({
      ...base,
      recommendations: Object.freeze([
    ...
    expect(propagationBroken.result.certificationState).toBe("FAIL");
    expect(propagationBroken.validation.reasonCodes).toContain("PROPAGATION_BROKEN");
  ...
  it("blocks cross-tenant and ownership mismatches", () => {
    const base = impactCertificationInput();
    const crossTenant = sealImpactCertification({
      ...base,
      recommendations: Object.freeze([
    ...
    const ownershipMismatch = sealImpactCertification({
      ...base,
      recommendations: Object.freeze([
  ...
  it("blocks execution, mutation, prioritization, approval, workflow routing, and authority expansion", () => {
    const base = impactCertificationInput();
    expect(sealImpactCertification({ ...base, executionRequested: true }).validation.reasonCodes).toContain("EXECUTION_REQUEST_BLOCKED");
    expect(sealImpactCertification({ ...base, certificationMutationAttempted: true }).validation.reasonCodes).toContain("CERTIFICATION_MUTATION_DETECTED");
    expect(sealImpactCertification({ ...base, prioritizationRequested: true }).validation.reasonCodes).toContain("PRIORITIZATION_DETECTED");
    expect(sealImpactCertification({ ...base, approvalRequested: true }).validation.reasonCodes).toContain("APPROVAL_DETECTED");
    expect(sealImpactCertification({ ...base, workflowRoutingRequested: true }).validation.reasonCodes).toContain("WORKFLOW_ROUTING_DETECTED");
    expect(sealImpactCertification({ ...base, authorityExpansionDetected: true }).validation.reasonCodes).toContain("AUTHORITY_EXPANSION_DETECTED");
  });

  it("fails when control surfaces expand through upstream artifacts", () => {
    const base = impactCertificationInput();
    const controlSurface = sealImpactCertification({
      ...base,
      replay: {
        ...base.replay,
        controlSurfacePresent: true,
      },
    });
    expect(controlSurface.result.certificationState).toBe("FAIL");
    expect(controlSurface.validation.reasonCodes).toContain("CONTROL_SURFACE_DETECTED");
  });
});
