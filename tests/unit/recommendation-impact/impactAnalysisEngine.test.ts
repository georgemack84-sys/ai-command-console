...
import {
  ...
  createImpactAnalysisEvidencePath,
  sealImpactAnalysis,
...
function portfolioInput(overrides: Partial<RecommendationPortfolioInput> = {}): RecommendationPortfolioInput {
  const alpha = buildBundle("recommendation-alpha", "graph-513b-alpha");
  const beta = buildBundle("recommendation-beta", "graph-513b-beta");
...
function impactAnalysisInput(overrides: Partial<ImpactAnalysisInput> = {}): ImpactAnalysisInput {
...
describe("impactAnalysisEngine", () => {
  it("is deterministic and reproduces analysis hashes", () => {
    ...
    const first = sealImpactAnalysis(input);
    const second = sealImpactAnalysis(input);
  ...
  it("keeps analysis ordering deterministic", () => {
    const input = impactAnalysisInput();
    const reversed = impactAnalysisInput({ recommendations: Object.freeze([...input.recommendations].reverse()) });
    expect(sealImpactAnalysis(reversed)).toEqual(sealImpactAnalysis(input));
    const sealed = sealImpactAnalysis(input);
    expect(createImpactAnalysisEvidencePath(input, sealed.evidencePath.chainReferences, sealed.evidencePath.propagationReferences, sealed.evidencePath.concentrationReferences, sealed.evidencePath.gapReferences, sealed.evidencePath.conflictReferences)).toEqual(
      createImpactAnalysisEvidencePath(input, sealed.evidencePath.chainReferences, sealed.evidencePath.propagationReferences, sealed.evidencePath.concentrationReferences, sealed.evidencePath.gapReferences, sealed.evidencePath.conflictReferences),
    );
  });

  it("detects impact chains, propagation, and concentrations reproducibly", () => {
    const sealed = sealImpactAnalysis(impactAnalysisInput());
    expect(sealed.result.impactChainsDetected).toBeGreaterThan(0);
    expect(sealed.result.propagationPathsDetected).toBeGreaterThan(0);
    expect(sealed.result.impactConcentrationsDetected).toBeGreaterThanOrEqual(0);
  });

  it("surfaces impact gaps and conflicts as LIMITED", () => {
    const base = impactAnalysisInput();
    const limited = sealImpactAnalysis({
      ...base,
      foundation: { ...base.foundation, result: { ...base.foundation.result, impactState: "LIMITED" } },
      dependencyReplay: { ...base.dependencyReplay, result: { ...base.dependencyReplay.result, replayState: "LIMITED" } },
    });
    expect(limited.result.analysisState).toBe("LIMITED");
  });

  it("surfaces missing impact evidence as OBSERVE", () => {
    const base = impactAnalysisInput();
    const observe = sealImpactAnalysis({
      ...base,
  ...
  it("blocks cross-tenant impacts and ownership mismatch", () => {
    const base = impactAnalysisInput();
    const crossTenant = sealImpactAnalysis({
      ...base,
    ...
    const ownershipMismatch = sealImpactAnalysis({
      ...base,
      recommendations: Object.freeze([
  ...
  it("fails on governance or replay corruption", () => {
    const base = impactAnalysisInput();
    const brokenGovernance = sealImpactAnalysis({
      ...base,
      recommendations: Object.freeze([
    ...
    const brokenReplay = sealImpactAnalysis({
      ...base,
  ...
  it("blocks execution, mutation, workflow ordering, approval, prioritization, and authority expansion", () => {
    const base = impactAnalysisInput();
    expect(sealImpactAnalysis({ ...base, executionRequested: true }).validation.reasonCodes).toContain("EXECUTION_REQUEST_BLOCKED");
    expect(sealImpactAnalysis({ ...base, analysisMutationAttempted: true }).validation.reasonCodes).toContain("ANALYSIS_MUTATION_DETECTED");
    expect(sealImpactAnalysis({ ...base, workflowRoutingRequested: true }).validation.reasonCodes).toContain("WORKFLOW_ROUTING_DETECTED");
    expect(sealImpactAnalysis({ ...base, approvalRequested: true }).validation.reasonCodes).toContain("APPROVAL_DETECTED");
    expect(sealImpactAnalysis({ ...base, prioritizationRequested: true }).validation.reasonCodes).toContain("PRIORITIZATION_DETECTED");
    expect(sealImpactAnalysis({ ...base, authorityExpansionDetected: true }).validation.reasonCodes).toContain("AUTHORITY_EXPANSION_DETECTED");
  });
});
