...
describe("constraintReplayFramework", () => {
  it("is deterministic and produces stable replay hashes", () => {
    const input = constraintReplayInput();
    const first = sealConstraintReplay(input);
    const second = sealConstraintReplay(input);
  ...
  it("keeps reconstruction ordering deterministic when recommendations are reversed", () => {
    const base = certifiableInput();
    ...
    const first = sealConstraintReplay(constraintReplayInput());
    const secondInput = constraintReplayInput({
      recommendations: reversedRecommendations,
      foundation,
      analysis,
      observability,
    });
    const second = sealConstraintReplay(secondInput);

    expect(second.evidencePath).toEqual(first.evidencePath);
    expect(second.result.replayHash).toBe(first.result.replayHash);
    expect(second.result.reconstructionHash).toBe(first.result.reconstructionHash);
  }, 30000);

  it("reconstructs LOW, MODERATE, HIGH, CRITICAL, and BLOCKING severities reproducibly", () => {
    const base = constraintAnalysisInput();
    const low = sealConstraintReplay(constraintReplayInput({ analysis: sealConstraintAnalysis(base), observability: sealConstraintObservability(constraintObservabilityInput({ analysis: sealConstraintAnalysis(base) })) }));
    ...
    expect(low.result.severityReconstructed).toBe(true);
    expect(sealConstraintReplay(constraintReplayInput({ analysis: moderateAnalysis, observability: sealConstraintObservability(constraintObservabilityInput({ analysis: moderateAnalysis })) })).result.severityReconstructed).toBe(true);
    expect(sealConstraintReplay(constraintReplayInput({ analysis: highAnalysis, observability: sealConstraintObservability(constraintObservabilityInput({ analysis: highAnalysis })) })).result.severityReconstructed).toBe(true);
    expect(sealConstraintReplay(constraintReplayInput({ analysis: criticalAnalysis, observability: sealConstraintObservability(constraintObservabilityInput({ analysis: criticalAnalysis })) })).result.severityReconstructed).toBe(true);
    expect(sealConstraintReplay(constraintReplayInput({ analysis: blockingAnalysis, observability: sealConstraintObservability(constraintObservabilityInput({ analysis: blockingAnalysis })) })).result.severityReconstructed).toBe(true);
  }, 60000);

  it("reconstructs propagation and conflicts reproducibly", () => {
    const sealed = sealConstraintReplay(constraintReplayInput());
    expect(sealed.result.propagationReconstructed).toBe(true);
    expect(sealed.result.conflictsReconstructed).toBe(true);
    expect(sealed.validation.reasonCodes).toContain("PROPAGATION_RECONSTRUCTED");
    expect(sealed.validation.reasonCodes).toContain("CONFLICTS_RECONSTRUCTED");
  }, 15000);

  it("drops to LIMITED when replay artifacts are missing", () => {
    const base = certifiableInput();
    ...
    expect(sealed.result.replayState).toBe("LIMITED");
    expect(sealed.validation.reasonCodes).toContain("REPLAY_ARTIFACTS_MISSING");
  }, 20000);

  it("escalates when replay hashes mismatch", () => {
    const base = constraintReplayInput();
    const sealed = sealConstraintReplay({
      ...base,
      dependencyReplay: {
    ...
    expect(sealed.result.replayState).toBe("ESCALATED");
    expect(sealed.validation.reasonCodes).toContain("REPLAY_HASH_MISMATCH");
  }, 15000);

  it("blocks cross-tenant replay and ownership mismatches", () => {
    const base = constraintReplayInput();
    const crossTenant = sealConstraintReplay({
      ...base,
      dependencyRiskReplay: {
        ...base.dependencyRiskReplay,
        result: {
          ...base.dependencyRiskReplay.result,
    ...
    const ownershipMismatch = sealConstraintReplay({
      ...base,
      recommendations: mismatchedRecommendations,
      foundation,
      analysis,
    ...
    expect(ownershipMismatch.result.replayState).toBe("INVALID");
  ...
  it("blocks execution, mutation, routing, approval, prioritization, ranking, scoring, resource allocation, and authority expansion", () => {
    const base = constraintReplayInput();
    expect(sealConstraintReplay({ ...base, executionRequested: true }).validation.reasonCodes).toContain("EXECUTION_REQUEST_BLOCKED");
    expect(sealConstraintReplay({ ...base, replayMutationAttempted: true }).validation.reasonCodes).toContain("REPLAY_MUTATION_DETECTED");
    expect(sealConstraintReplay({ ...base, workflowRoutingRequested: true }).validation.reasonCodes).toContain("WORKFLOW_ROUTING_DETECTED");
    expect(sealConstraintReplay({ ...base, prioritizationRequested: true }).validation.reasonCodes).toContain("PRIORITIZATION_DETECTED");
    expect(sealConstraintReplay({ ...base, recommendationRankingRequested: true }).validation.reasonCodes).toContain("RANKING_DETECTED");
    expect(sealConstraintReplay({ ...base, approvalRequested: true }).validation.reasonCodes).toContain("APPROVAL_DETECTED");
    expect(sealConstraintReplay({ ...base, recommendationScoringRequested: true }).validation.reasonCodes).toContain("SCORING_DETECTED");
    expect(sealConstraintReplay({ ...base, resourceAllocationRequested: true }).validation.reasonCodes).toContain("RESOURCE_ALLOCATION_DETECTED");
    expect(sealConstraintReplay({ ...base, authorityExpansionDetected: true }).validation.reasonCodes).toContain("AUTHORITY_EXPANSION_DETECTED");
  }, 15000);
  ...
  it("fails on governance corruption", () => {
    ...
    const brokenGovernance = sealConstraintReplay({
      ...base,
      recommendations: brokenRecommendations,
      foundation,
      analysis,
