...
import {
  ...
  createConstraintObservabilityEvidencePath,
...
describe("constraintObservabilityLayer", () => {
  it("is deterministic and produces stable observability hashes", () => {
    const input = constraintObservabilityInput();
    const first = sealConstraintObservability(input);
    const second = sealConstraintObservability(input);
  ...
  it("keeps visibility ordering deterministic when recommendations are reversed", () => {
    ...
    const first = sealConstraintObservability(constraintObservabilityInput({ recommendations: base.recommendations }));
    const secondInput = constraintObservabilityInput({
    ...
    const second = sealConstraintObservability({
      ...secondInput,
      analysis: sealConstraintAnalysis({
        ...constraintAnalysisInput({
          recommendations: reversedRecommendations,
          foundation: secondInput.foundation,
        }),
      }),
    });

    expect(second.evidencePath).toEqual(first.evidencePath);
    expect(second.result.observabilityHash).toBe(first.result.observabilityHash);
    expect(createConstraintObservabilityEvidencePath(secondInput)).toEqual(first.evidencePath);
  }, 20000);

  it("makes LOW, MODERATE, HIGH, CRITICAL, and BLOCKING severities visible", () => {
    const base = constraintAnalysisInput();
    const lowVisible = sealConstraintObservability(constraintObservabilityInput({ analysis: sealConstraintAnalysis(base) }));
    const moderateVisible = sealConstraintObservability(constraintObservabilityInput({
      analysis: sealConstraintAnalysis({
        ...base,
        trustFoundation: {
    ...
    const highVisible = sealConstraintObservability(constraintObservabilityInput({
      analysis: sealConstraintAnalysis({
        ...base,
        dependencyRiskFoundation: {
    ...
    const criticalVisible = sealConstraintObservability(constraintObservabilityInput({
      analysis: sealConstraintAnalysis({
        ...base,
        trustFoundation: {
          ...base.trustFoundation,
    ...
    const blockingVisible = sealConstraintObservability(constraintObservabilityInput({
      analysis: sealConstraintAnalysis({
        ...base,
        resilienceFoundation: {
    ...
    expect(lowVisible.result.constraintSeverityVisible).toBe(true);
    expect(moderateVisible.result.constraintSeverityVisible).toBe(true);
    expect(highVisible.result.constraintSeverityVisible).toBe(true);
    expect(criticalVisible.result.constraintSeverityVisible).toBe(true);
    expect(blockingVisible.result.constraintSeverityVisible).toBe(true);
  }, 30000);

  it("surfaces propagation, lineage, governance, replay, and audit visibility reproducibly", () => {
    const sealed = sealConstraintObservability(constraintObservabilityInput());
    expect(sealed.result.constraintPropagationVisible).toBe(true);
    expect(sealed.result.constraintLineageVisible).toBe(true);
    expect(sealed.result.constraintGovernanceVisible).toBe(true);
    expect(sealed.result.constraintReplayVisible).toBe(true);
    expect(sealed.result.constraintAuditVisible).toBe(true);
  });

  it("drops to OBSERVE when visibility evidence is incomplete", () => {
    const base = constraintAnalysisInput();
    const observedAnalysis = sealConstraintAnalysis({
      ...base,
      foundation: {
        ...base.foundation,
    ...
    expect(sealed.result.observabilityState).toBe("OBSERVE");
    expect(sealed.validation.reasonCodes).toContain("VISIBILITY_EVIDENCE_MISSING");
  });

  it("drops to LIMITED when replay visibility is degraded", () => {
    const base = certifiableInput();
    const degradedRecommendations = Object.freeze([
      {
    ...
    const foundation = sealRecommendationConstraintFoundation({
      ...base,
      recommendations: degradedRecommendations,
    });
    const degradedFoundation = {
      ...foundation,
      evidencePath: {
        ...foundation.evidencePath,
        replayReferences: [],
      },
    };
    const sealed = sealConstraintObservability(constraintObservabilityInput({
      foundation: degradedFoundation,
      recommendations: degradedRecommendations,
      analysis: sealConstraintAnalysis(constraintAnalysisInput({
        foundation: degradedFoundation,
        recommendations: degradedRecommendations,
      })),
    }));

    expect(sealed.result.observabilityState).toBe("LIMITED");
    expect(sealed.result.constraintReplayVisible).toBe(false);
  }, 20000);

  it("blocks cross-tenant visibility and ownership mismatches", () => {
    const base = constraintObservabilityInput();
    const crossTenant = sealConstraintObservability({
      ...base,
      dependencyRiskFoundation: {
        ...base.dependencyRiskFoundation,
        result: {
    ...
    const ownershipMismatch = sealConstraintObservability({
      ...base,
    ...
    expect(ownershipMismatch.result.observabilityState).toBe("INVALID");
  ...
  it("blocks execution, mutation, routing, approval, prioritization, ranking, scoring, resource allocation, and authority expansion", () => {
    const base = constraintObservabilityInput();
    expect(sealConstraintObservability({ ...base, executionRequested: true }).validation.reasonCodes).toContain("EXECUTION_REQUEST_BLOCKED");
    expect(sealConstraintObservability({ ...base, observabilityMutationAttempted: true }).validation.reasonCodes).toContain("OBSERVABILITY_MUTATION_DETECTED");
    expect(sealConstraintObservability({ ...base, workflowRoutingRequested: true }).validation.reasonCodes).toContain("WORKFLOW_ROUTING_DETECTED");
    expect(sealConstraintObservability({ ...base, prioritizationRequested: true }).validation.reasonCodes).toContain("PRIORITIZATION_DETECTED");
    expect(sealConstraintObservability({ ...base, recommendationRankingRequested: true }).validation.reasonCodes).toContain("RANKING_DETECTED");
    expect(sealConstraintObservability({ ...base, approvalRequested: true }).validation.reasonCodes).toContain("APPROVAL_DETECTED");
    expect(sealConstraintObservability({ ...base, recommendationScoringRequested: true }).validation.reasonCodes).toContain("SCORING_DETECTED");
    expect(sealConstraintObservability({ ...base, resourceAllocationRequested: true }).validation.reasonCodes).toContain("RESOURCE_ALLOCATION_DETECTED");
    expect(sealConstraintObservability({ ...base, authorityExpansionDetected: true }).validation.reasonCodes).toContain("AUTHORITY_EXPANSION_DETECTED");
  });
  ...
  it("fails on governance corruption", () => {
    ...
    const brokenGovernance = sealConstraintObservability({
      ...base,
      recommendations: brokenRecommendations,
      foundation,
      analysis: sealConstraintAnalysis(constraintAnalysisInput({
