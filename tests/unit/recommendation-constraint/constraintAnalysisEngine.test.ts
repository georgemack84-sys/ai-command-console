...
import {
  ...
  createConstraintAnalysisEvidencePath,
...
describe("constraintAnalysisEngine", () => {
  it("is deterministic and produces stable analysis hashes", () => {
    const input = constraintAnalysisInput();
    const first = sealConstraintAnalysis(input);
    const second = sealConstraintAnalysis(input);
  ...
  it("keeps analysis ordering deterministic when recommendations are reversed", () => {
    ...
    const first = sealConstraintAnalysis(constraintAnalysisInput({ recommendations: base.recommendations }));
    const second = sealConstraintAnalysis(constraintAnalysisInput({
      recommendations: reversedRecommendations,
      foundation: sealRecommendationConstraintFoundation({
        ...base,
        recommendations: reversedRecommendations,
      }),
    }));

    expect(second.severities).toEqual(first.severities);
    expect(second.concentrations).toEqual(first.concentrations);
    expect(second.propagations).toEqual(first.propagations);
    expect(second.evidencePath).toEqual(first.evidencePath);
    expect(second.result.analysisHash).toBe(first.result.analysisHash);
    expect(createConstraintAnalysisEvidencePath(
      constraintAnalysisInput({ recommendations: reversedRecommendations, foundation: second.evidencePath ? sealRecommendationConstraintFoundation({ ...base, recommendations: reversedRecommendations }) : undefined }),
      second.severities,
      second.concentrations,
      second.propagations,
      second.gaps,
      second.conflicts,
    )).toEqual(first.evidencePath);
  }, 15000);

  it("surfaces LOW, MODERATE, HIGH, CRITICAL, and BLOCKING severities reproducibly", () => {
    const base = constraintAnalysisInput();
    const low = sealConstraintAnalysis(base);
    const moderate = sealConstraintAnalysis({
      ...base,
      trustFoundation: {
    ...
    const high = sealConstraintAnalysis({
      ...base,
      dependencyRiskFoundation: {
    ...
    const critical = sealConstraintAnalysis({
      ...base,
      trustFoundation: {
        ...base.trustFoundation,
        result: {
    ...
    const blocking = sealConstraintAnalysis({
      ...base,
      resilienceFoundation: {
    ...
    expect(new Set(low.severities.map((severity) => severity.severity))).toContain("LOW");
    expect(new Set(moderate.severities.map((severity) => severity.severity))).toContain("MODERATE");
    expect(new Set(high.severities.map((severity) => severity.severity))).toContain("HIGH");
    expect(new Set(critical.severities.map((severity) => severity.severity))).toContain("CRITICAL");
    expect(new Set(blocking.severities.map((severity) => severity.severity))).toContain("BLOCKING");
  }, 30000);

  it("surfaces concentration and propagation evidence reproducibly", () => {
    const sealed = sealConstraintAnalysis(constraintAnalysisInput());
    expect(sealed.result.constraintConcentrationsDetected).toBeGreaterThan(0);
    expect(sealed.result.constraintPropagationsDetected).toBeGreaterThan(0);
    expect(sealed.validation.reasonCodes).toContain("CONCENTRATIONS_ANALYZED");
    expect(sealed.validation.reasonCodes).toContain("PROPAGATIONS_ANALYZED");
  });

  it("surfaces gaps as LIMITED when evidence is missing", () => {
    const base = constraintAnalysisInput();
    const sealed = sealConstraintAnalysis({
      ...base,
      trustFoundation: {
        ...base.trustFoundation,
        result: {
    ...
    expect(sealed.result.analysisState).toBe("LIMITED");
    expect(sealed.result.constraintGapsDetected).toBeGreaterThan(0);
    expect(sealed.validation.reasonCodes).toContain("CONSTRAINT_GAPS_DETECTED");
  });

  it("surfaces conflicts as LIMITED", () => {
    const baseInput = constraintAnalysisInput();
    const sealed = sealConstraintAnalysis({
      ...baseInput,
      trustFoundation: {
        ...baseInput.trustFoundation,
        result: {
          ...baseInput.trustFoundation.result,
          trustState: "DEGRADED",
        },
      },
    });
    ...
    expect(sealed.result.analysisState).toBe("LIMITED");
    expect(sealed.result.constraintConflictsDetected).toBeGreaterThan(0);
    expect(sealed.validation.reasonCodes).toContain("CONSTRAINT_CONFLICTS_DETECTED");
  });

  it("blocks cross-tenant inputs and ownership mismatches", () => {
    const baseInput = constraintAnalysisInput();
    const crossTenant = sealConstraintAnalysis({
      ...baseInput,
      dependencyRiskFoundation: {
        ...baseInput.dependencyRiskFoundation,
        result: {
          ...baseInput.dependencyRiskFoundation.result,
          tenantIsolationVerified: false,
        },
      },
    });
    const ownershipMismatchRecommendations = Object.freeze([
      {
        ...baseInput.recommendations[0],
        ownershipEvidence: {
          ...baseInput.recommendations[0].ownershipEvidence,
          recommendationId: "recommendation-other",
        },
      },
      ...baseInput.recommendations.slice(1),
    ]);
    const ownershipMismatch = sealConstraintAnalysis({
      ...baseInput,
      recommendations: ownershipMismatchRecommendations,
      foundation: sealRecommendationConstraintFoundation({
        ...certifiableInput(),
        recommendations: ownershipMismatchRecommendations,
      }),
    ...
    expect(ownershipMismatch.result.analysisState).toBe("INVALID");
  ...
  it("blocks execution, mutation, routing, approval, prioritization, ranking, scoring, resource allocation, and authority expansion", () => {
    const base = constraintAnalysisInput();
    expect(sealConstraintAnalysis({ ...base, executionRequested: true }).validation.reasonCodes).toContain("EXECUTION_REQUEST_BLOCKED");
    expect(sealConstraintAnalysis({ ...base, analysisMutationAttempted: true }).validation.reasonCodes).toContain("ANALYSIS_MUTATION_DETECTED");
    expect(sealConstraintAnalysis({ ...base, workflowRoutingRequested: true }).validation.reasonCodes).toContain("WORKFLOW_ROUTING_DETECTED");
    expect(sealConstraintAnalysis({ ...base, prioritizationRequested: true }).validation.reasonCodes).toContain("PRIORITIZATION_DETECTED");
    expect(sealConstraintAnalysis({ ...base, recommendationRankingRequested: true }).validation.reasonCodes).toContain("RANKING_DETECTED");
    expect(sealConstraintAnalysis({ ...base, approvalRequested: true }).validation.reasonCodes).toContain("APPROVAL_DETECTED");
    expect(sealConstraintAnalysis({ ...base, recommendationScoringRequested: true }).validation.reasonCodes).toContain("SCORING_DETECTED");
    expect(sealConstraintAnalysis({ ...base, resourceAllocationRequested: true }).validation.reasonCodes).toContain("RESOURCE_ALLOCATION_DETECTED");
    expect(sealConstraintAnalysis({ ...base, authorityExpansionDetected: true }).validation.reasonCodes).toContain("AUTHORITY_EXPANSION_DETECTED");
  });

  it("fails on governance corruption", () => {
    const base = constraintAnalysisInput();
    const brokenRecommendations = Object.freeze([
      {
    ...
    const brokenGovernance = sealConstraintAnalysis({
      ...base,
      recommendations: brokenRecommendations,
      foundation: sealRecommendationConstraintFoundation({
        ...certifiableInput(),
        recommendations: brokenRecommendations,
      }),
    ...
    expect(brokenGovernance.result.analysisState).toBe("INVALID");
