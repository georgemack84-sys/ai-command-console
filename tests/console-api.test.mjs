...
const { handleConsoleRequest, queueDueDigestSweepIfNeeded } = require("../services/consoleApi");
const { runDigestSchedulerSweep, stopDigestScheduler } = require("../services/digestScheduler");
const { loadQueue, saveQueue, completeTask } = require("../services/taskQueue");
const { saveReviewState } = require("../services/reviewQueue");
const { saveAlertsState } = require("../services/alerts");
const { clearAuditEvents, listAuditEvents } = require("../services/auditTrail");
...
const { clearTelemetry, buildTelemetrySummary } = require("../services/telemetry");
const { clearJobs, runPendingJobs, enqueueJob, updateJob, registerJobProcessor } = require("../services/jobQueue");
...
const AGENT_PROFILE_PATH = path.join(process.cwd(), "agents", "researcher.json");
const ROUTES_STORE_PATH = path.join(process.cwd(), "data", "workspace-user-routes.json");
const BRIEFS_STORE_PATH = path.join(process.cwd(), "data", "research-briefs.json");
const REPORTS_STORE_PATH = path.join(process.cwd(), "data", "research-reports.json");
...
test("agent:update-config persists safe profile changes and exposes them in the overview", async () => {
  const snapshot = snapshotFiles(FILES);
  const originalProfile = fs.readFileSync(AGENT_PROFILE_PATH, "utf8");
...
test("due digest sweep generates background digests for eligible workspace users", async () => {
  const snapshot = snapshotFiles(FILES);
  const usersPath = path.join(process.cwd(), "data", "workspace-users.json");
