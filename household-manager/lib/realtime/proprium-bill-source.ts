...
async function request<T>(path: string, method: "POST" | "PATCH", body: object): Promise<T> {
  const source = propriumBillSource();
  if (!source) throw new Error("Proprium Bills are not configured.");
  const response = await fetch(`${source.origin}/api/v1/households/${source.householdId}/bills${path}`, { method, credentials: "include", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) });
  if (!response.ok) throw new Error(response.status === 401 ? "Sign in to Proprium to manage Bills." : "Unable to update Proprium Bills.");
