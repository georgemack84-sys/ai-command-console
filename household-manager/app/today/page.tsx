...
import { listPropriumBills, propriumBillSource, type PropriumBill } from "../../lib/realtime/proprium-bill-source";
...
export default function TodayPage() {
  ...
  const [propriumBills, setPropriumBills] = useState<PropriumBill[]>([]);
  ...
  const refresh = useCallback(async () => {
    try {
      const [location, savedTasks, savedChores, savedShopping, savedPropriumBills] = await Promise.all([locationService.getEffectiveLocation("HOUSEHOLD"), taskService.list(), householdService.listChores(), householdService.listShopping(), propriumBillSource() ? listPropriumBills() : Promise.resolve([])]);
      const [updatedForecast, response] = await Promise.all([weatherService.getForecast(location), fetch("/api/context/today")]);
      setForecast(updatedForecast); setTasks(savedTasks); setChores(savedChores); setShopping(savedShopping); setPropriumBills(savedPropriumBills);
  ...
  const propriumAttention = useMemo(() => propriumBills.filter((bill) => bill.paymentStatus === "Unpaid").flatMap((bill) => {
    const today = new Date();
    const localToday = new Date(today.getTime() - today.getTimezoneOffset() * 60_000).toISOString().slice(0, 10);
    const tomorrow = new Date(today); tomorrow.setDate(tomorrow.getDate() + 1);
    const localTomorrow = new Date(tomorrow.getTime() - tomorrow.getTimezoneOffset() * 60_000).toISOString().slice(0, 10);
    if (bill.dueDate > localTomorrow) return [];
    const overdue = bill.dueDate < localToday;
    const dueToday = bill.dueDate === localToday;
    return [{ id: `proprium-bill:${bill.id}`, title: overdue ? `${bill.name} is overdue` : `${bill.name} needs attention`, message: `$${bill.amount.toFixed(2)} · ${overdue ? "Past due" : dueToday ? "Due today" : "Due tomorrow"}` }];
  }), [propriumBills]);
  const attention = useMemo(() => [...(context?.attention ?? []), ...propriumAttention], [context?.attention, propriumAttention]);
