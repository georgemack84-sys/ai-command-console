...
export function ReportsPageClient() {
  ...
  return (
    <div className="space-y-6">
      <SectionCard
        eyebrow="Reports"
        title="Synthesis and publication"
        description="Turn queued briefs into memos, briefings, comparisons, and outlines with a lightweight publishing surface."
      >
        <div className="grid gap-6 xl:grid-cols-[0.95fr_1.05fr]">
          <div className="rounded-[28px] border border-white/10 bg-black/25 p-5">
            <p className="text-sm font-medium text-white">Create a report</p>
            <div className="mt-4 space-y-3">
              <select value={form.briefId} onChange={(event) => setForm((current) => ({ ...current, briefId: event.target.value }))} className="w-full rounded-xl border border-white/10 bg-slate-950/80 px-3 py-2 text-sm text-white outline-none">
              ...
              <input value={form.title} onChange={(event) => setForm((current) => ({ ...current, title: event.target.value }))} className="w-full rounded-xl border border-white/10 bg-slate-950/80 px-3 py-2 text-sm text-white outline-none" placeholder="Report title" />
              <select value={form.format} onChange={(event) => setForm((current) => ({ ...current, format: event.target.value as ResearchReport["format"] }))} className="w-full rounded-xl border border-white/10 bg-slate-950/80 px-3 py-2 text-sm text-white outline-none">
                <option value="memo">Memo</option>
                <option value="briefing">Briefing</option>
                <option value="comparison">Comparison</option>
                <option value="outline">Outline</option>
              </select>
              <textarea value={form.excerpt} onChange={(event) => setForm((current) => ({ ...current, excerpt: event.target.value }))} className="min-h-24 w-full rounded-xl border border-white/10 bg-slate-950/80 px-3 py-2 text-sm text-white outline-none" placeholder="Short summary or excerpt" />
              <textarea value={form.keyFindings} onChange={(event) => setForm((current) => ({ ...current, keyFindings: event.target.value }))} className="min-h-28 w-full rounded-xl border border-white/10 bg-slate-950/80 px-3 py-2 text-sm text-white outline-none" placeholder="One key finding per line" />
              <div className="flex flex-wrap gap-3">
                <button type="button" onClick={() => void createReport()} disabled={saving} className="rounded-full bg-cyan-300 px-4 py-2 text-sm font-semibold text-slate-950 disabled:opacity-60">
          ...
          <div className="space-y-4">
            ...
            {visibleReports.length ? (
                ...
                <article key={report.id} className="rounded-[28px] border border-white/10 bg-[linear-gradient(180deg,rgba(8,19,31,0.88),rgba(8,19,31,0.72))] p-5">
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    ...
                    <div className="flex flex-wrap gap-2">
                      ...
                      <span className="rounded-full border border-white/10 bg-white/5 px-3 py-1 text-xs text-slate-200">{report.format}</span>
                    </div>
                  </div>
                  ...
                  {currentUser?.role === "admin" && workspaceUsers.length ? (
                    <div className="mt-4 flex flex-wrap items-center gap-2">
                      <select
                        ...
                        onChange={(event) =>
                          setOwnerDrafts((current) => ({ ...current, [report.id]: event.target.value }))
                        }
                      ...
                      <button
                        type="button"
                        onClick={() => void reassignReport(report.id)}
                  ...
                  <div className="mt-4 flex flex-wrap gap-2">
                    {(["draft", "ready", "published"] as ResearchReport["status"][]).map((status) => (
                      <button key={status} type="button" onClick={() => void updateReportStatus(report.id, status)} className="rounded-full border border-white/10 bg-white/5 px-3 py-2 text-xs text-white transition hover:bg-white/10">
                        Mark {status}
                      </button>
                    ))}
                    <button type="button" onClick={() => void deleteReport(report.id)} className="rounded-full border border-rose-400/20 bg-rose-400/10 px-3 py-2 text-xs text-rose-100">
                      Delete
                    </button>
                  </div>
                </article>
              ...
              <div className="rounded-3xl border border-dashed border-white/10 bg-white/5 p-6 text-sm text-slate-300">
                {loading ? "Loading reports..." : "No reports match this ownership filter yet."}
              </div>
            )}
          </div>
        </div>
