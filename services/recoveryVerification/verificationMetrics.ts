...
export function recordVerificationMetric(metric: string, tenantId?: string | null, increment = 1) {
  const metricKey = key(metric, tenantId);
  counters.set(metricKey, (counters.get(metricKey) || 0) + increment);
}

export function getVerificationMetrics(tenantId?: string | null) {
...
export function resetVerificationMetricsForTests() {
  counters.clear();
}
