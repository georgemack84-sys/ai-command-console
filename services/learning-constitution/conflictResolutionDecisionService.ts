...
export class ConflictResolutionDecisionService {
  ...
  async decide(input: Readonly<{ conflictId: string; proposalId: string; acceptedOutcome: ConflictResolutionProposal["proposedOutcome"]; decisionMaker: ProvenanceActor; decisionAuthority: string; decisionReason: string; approvalRef?: string; executionPlan?: ConflictResolutionDecision["executionPlan"] }>): Promise<ConflictResolutionDecision | undefined> {
