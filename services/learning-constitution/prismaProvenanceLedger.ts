...
type PrismaTransactionClient = PrismaProvenanceClient & Readonly<{ $transaction<T>(operation: (client: PrismaProvenanceClient) => Promise<T>): Promise<T> }>;
...
const provenanceClient = prisma as unknown as PrismaTransactionClient;
...
export class PrismaProvenanceLedger implements TransactionalProvenanceLedger {
  constructor(private readonly workspaceId: string, private readonly client: PrismaProvenanceClient = provenanceClient) {
  ...
  async append(record: ProvenanceRecord): Promise<ProvenanceRecord> {
    const existing = await this.client.noesisProvenanceRecord.findUnique({ where: { recordId: record.id } });
    if (existing) {
    ...
    await this.client.noesisProvenanceRecord.create({ data: { recordId: record.id, workspaceId: this.workspaceId, recordType: record.recordType, payload: record as unknown as object } });
    return record;
  }

  async relate(relationship: ProvenanceRelationship): Promise<ProvenanceRelationship> {
    const existing = await this.client.noesisProvenanceRelationship.findUnique({ where: { relationshipId: relationship.id } });
    if (existing) {
  ...
  async getRelationships(recordId: string): Promise<readonly ProvenanceRelationship[]> {
    return (await this.client.noesisProvenanceRelationship.findMany({ where: { workspaceId: this.workspaceId, OR: [{ fromRecordId: recordId }, { toRecordId: recordId }] }, orderBy: [{ createdAt: "asc" }, { relationshipId: "asc" }] })).map(toRelationship);
  }
  async getAll(): Promise<readonly ProvenanceRecord[]> {
    return (await this.client.noesisProvenanceRecord.findMany({ where: { workspaceId: this.workspaceId }, orderBy: [{ createdAt: "asc" }, { recordId: "asc" }] })).map(toRecord);
  }
  async withTransaction<T>(operation: (ledger: ProvenanceLedger) => Promise<T>): Promise<T> {
    return provenanceClient.$transaction((client) => operation(new PrismaProvenanceLedger(this.workspaceId, client)));
  }
}
