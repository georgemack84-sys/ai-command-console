function normalize(value: unknown, seen: WeakSet<object>): unknown {
  ...
  if (typeof value === "bigint" || typeof value === "function" || typeof value === "symbol") {
    throw new Error("DECISION_READINESS_UNSUPPORTED_STRUCTURE");
  }
  if (Array.isArray(value)) return value.map((entry) => normalize(entry, seen));
  if (value && typeof value === "object") {
    if (seen.has(value as object)) throw new Error("DECISION_READINESS_CIRCULAR_STRUCTURE");
...
export function canonicalizeCertificationValue<T>(value: T): T {
