...
function packageStandaloneAssets() {
  const standaloneRoot = path.join(productionDistDir, "standalone");
  const staticSource = path.join(productionDistDir, "static");
  const staticDestination = path.join(standaloneRoot, ".next-production", "static");
  const publicSource = path.join(root, "public");
  const publicDestination = path.join(standaloneRoot, "public");

  if (!fs.existsSync(standaloneRoot)) {
    throw new Error("Standalone output was not generated by the production build.");
  }
  if (!fs.existsSync(staticSource)) {
    throw new Error("Production static assets were not generated by the build.");
  }
  ...
  console.log("Standalone packaging: copied production static and public assets.");
}
...
if (command === "standalone") {
  const standaloneServer = path.join(productionDistDir, "standalone", "server.js");
...
// Next 15 uses webpack by default; avoid the Next 16-only --webpack flag.
...
if (command === "build" && (result.status ?? 1) === 0) {
  try {
    packageStandaloneAssets();
  } catch (error) {
