...
test('core component validation delegates to the frontend package', () => {
  const validation = recorder();
  execute('validate components', { spawn: validation.spawn, log() {} });
  assert.deepEqual(npmArguments(validation.calls[0]), [
    'run',
    'validate:components',
  ]);
...
test('overlay validation delegates to the frontend package', () => {
  const validation = recorder();
  execute('validate overlays', { spawn: validation.spawn, log() {} });
  assert.deepEqual(npmArguments(validation.calls[0]), [
    'run',
    'validate:overlays',
  ]);
...
test('route-state validation delegates to the frontend package', () => {
  const validation = recorder();
  execute('validate route-states', { spawn: validation.spawn, log() {} });
  assert.deepEqual(npmArguments(validation.calls[0]), [
    'run',
    'validate:route-states',
  ]);
...
test('Week 2 qualification delegates to the frontend package', () => {
  const validation = recorder();
  execute('validate week-2', { spawn: validation.spawn, log() {} });
  assert.deepEqual(npmArguments(validation.calls[0]), [
    'run',
    'validate:week-2',
  ]);
...
test('authentication UI validation delegates to the frontend package', () => {
  const validation = recorder();
  execute('validate authentication-ui', {
    spawn: validation.spawn,
    log() {},
  });
  assert.deepEqual(npmArguments(validation.calls[0]), [
