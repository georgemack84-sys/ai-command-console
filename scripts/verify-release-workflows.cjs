...
assert.match(stagingDeploySource, /proprium-postgres/);
assert.match(stagingDeploySource, /proprium-redis/);
assert.match(stagingDeploySource, /wait_for_healthy/);
assert.match(stagingDeploySource, /local max_attempts="\$\{2:-30\}"/);
assert.match(stagingDeploySource, /compose run --rm --no-deps -T --interactive=false database-migrations/);
assert.match(stagingDeploySource, /disable both TTY allocation and interactivity/);
assert.match(stagingDeploySource, /platform-api/);
assert.match(stagingDeploySource, /web/);
assert.match(stagingDeploySource, /wait_for_healthy web 60/);
assert.match(stagingDeploySource, /current-release\.env/);
assert.match(stagingDeploySource, /test -f "\$runtime_dir\/current-release\.env"/);
...
assert.match(stagingBootstrapSource, /generate_isolated/);
assert.match(stagingBootstrapSource, /random_base64/);
